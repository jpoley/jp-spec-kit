...
**Feature**: Enable jpspec and speckit slash commands in VS Code Copilot Chat
**Status**: Draft
**Created**: 2025-12-07
...
1. **Mirroring commands** from `.claude/commands/jpspec/*.md` and `.claude/commands/speckit/*.md` to `.github/agents/jpspec.*.md` and `.github/agents/speckit.*.md`
2. **Transforming frontmatter** from `mode: agent` to `mode: jpspec.<name>` or `mode: speckit.<name>`
...
- **Workflow**: Uses `/jpspec:specify` to create PRDs, `/jpspec:implement` to generate code
- **Pain Point**: Cannot access jpspec commands in Copilot Chat - must switch to terminal

**Persona 2: Platform Engineer**
- **Name**: Jordan Martinez
- **Environment**: VS Code Insiders (testing pre-release features)
- **Workflow**: Uses `/jpspec:plan` for architecture design, `/jpspec:operate` for SRE tasks
...
- **Pain Point**: Cannot use jpspec commands without installing CLI tools - wants IDE-only experience
...
3. No jpspec commands appear ❌
4. Alex switches to terminal, runs `specify` CLI
5. Copies output back to VS Code

Desired State (Fixed):
1. Alex opens VS Code Copilot Chat
2. Types "/jpspec:" and sees autocomplete list ✅
3. Selects /jpspec:specify, provides feature description
...
2. All jpspec commands available via Copilot Chat
...
I want to see all jpspec and speckit slash commands in the Copilot Chat command picker,
So that I can discover and use SDD workflows without leaving my IDE.

Acceptance Criteria:
- When I type "/" in Copilot Chat, I see commands grouped by prefix (jpspec, speckit)
- Each command shows a brief description (from frontmatter)
- Commands are filtered as I type (e.g., "/jpspec:sp" filters to specify, speckit.specify)
...
```
As a developer,
I want jpspec commands to execute correctly when invoked in Copilot Chat,
So that I can create specs, generate code, and validate work without CLI tools.

Acceptance Criteria:
- /jpspec:specify creates a PRD in docs/prd/ with proper frontmatter
- /jpspec:implement generates implementation code with tests
- /speckit:plan creates architecture decision records in docs/adr/
- All commands respect jpspec_workflow.yml state transitions
...
- Script transforms frontmatter: mode: agent → mode: jpspec.<name>
...
- **Scenario**: VS Code has native slash commands that conflict with jpspec
- **Handling**: Use namespaced format (`jpspec:specify` not just `specify`)
- **Validation**: Check VS Code Copilot docs for reserved command names

**Edge Case 2: Frontmatter Include Directives**
- **Scenario**: Commands use `{{INCLUDE:.claude/commands/jpspec/_backlog-instructions.md}}`
...
- **Error Message**: "Invalid frontmatter in .github/agents/jpspec.specify.md: mode must be 'jpspec.specify', found 'agent'"
...
**Question**: Will developers actually use jpspec commands in VS Code Copilot, or do they prefer the CLI?

**Risk Level**: LOW

**Validation Plan**:
- **Concierge Test**: Manually convert 3 commands (specify, implement, plan) for early testing
- **Prototype**: Create .github/agents/ for jpspec.specify only, share with 5 beta users
- **Success Criteria**: 3 out of 5 users prefer VS Code Copilot over CLI for spec creation

**Mitigation**:
- Survey current jpspec users: "Would you use jpspec commands in VS Code Copilot if available?"
- Hypothesis: Developers already using Copilot Chat prefer in-IDE workflows over CLI

### Usability Risk (Experience)

**Question**: Can users figure out how to discover and invoke jpspec commands in Copilot Chat?

**Risk Level**: LOW

**Validation Plan**:
- **Usability Test**: Ask 3 developers to "create a feature spec using jpspec" without instructions
- **Metrics**: Time to discover command, number of failed attempts, need for documentation
- **Success Criteria**: All 3 users find and execute `/jpspec:specify` in < 2 minutes

**Mitigation**:
- Add README section: "Using jpspec with VS Code Copilot"
- Include screenshot of Copilot Chat command picker showing jpspec commands
...
- **Technical Constraints**:
  ...
  - File naming conventions (jpspec/specify.md vs jpspec.specify.md)
...
- **Risk**: Command naming conflicts → Solution: Use namespaced prefixes (jpspec:, speckit:)
...
└── agents/
    ...
    ├── jpspec.init.md
    ├── jpspec.reset.md
    ├── jpspec.security_fix.md
    ├── jpspec.security_report.md
    ├── jpspec.security_triage.md
    ├── jpspec.security_web.md
    ├── jpspec.security_workflow.md
    ├── speckit.specify.md
    ├── speckit.plan.md
    ├── speckit.tasks.md
...
**Input** (Claude Code format - `.claude/commands/jpspec/specify.md`):
```yaml
---
...
**Output** (GitHub Copilot format - `.github/agents/jpspec.specify.md`):
```yaml
---
description: Create or update feature specifications using PM planner agent
mode: jpspec.specify
...
1. `mode: agent` → `mode: <namespace>.<command>` (namespace = jpspec or speckit)
...
Many jpspec commands include shared prompt fragments:
```markdown
...
2. Read referenced file content (relative to `.claude/commands/jpspec/`)
3. Replace directive with file content inline
4. Preserve markdown formatting and indentation

**Example**:

**Source** (`.claude/commands/jpspec/specify.md`):
```markdown
...
**Output** (`.github/agents/jpspec.specify.md`):
```markdown
## Execution Instructions
...
```bash
#!/usr/bin/env bash
# Converts .claude/commands/jpspec/*.md and .claude/commands/speckit/*.md
...
2. For each `.claude/commands/jpspec/*.md` (excluding `_*.md` fragments):
   - Read frontmatter and content
   - Transform frontmatter (mode: agent → mode: jpspec.<name>)
   - Resolve all `{{INCLUDE:...}}` directives
   - Write to `.github/agents/jpspec.<name>.md`
3. Repeat for `.claude/commands/speckit/*.md`
4. Output summary: "Converted 23 commands (15 jpspec, 8 speckit)"
...
**User Workflow**:
1. Developer edits `.claude/commands/jpspec/specify.md`
2. Runs `git add .claude/commands/jpspec/specify.md`
3. Runs `git commit -m "Update specify command"`
4. Pre-commit hook:
   - Detects change to .claude/commands/
   - Runs sync script
   - Auto-stages .github/agents/jpspec.specify.md
...
- Hook shows clear output: "Synced jpspec.specify.md to .github/agents/"
...
3. Command output matches expected behavior (e.g., /jpspec:specify creates PRD)
4. Include directives are resolved (no `{{INCLUDE:...}}` in output)
5. Workflow state transitions work (jpspec_workflow.yml integration)
...
```bash
specify --help
# Use /jpspec:specify, /jpspec:implement, etc.
```

### In VS Code Copilot Chat
1. Open Copilot Chat panel (Cmd+Shift+I or Ctrl+Shift+I)
2. Type "/" to see available commands
3. Select /jpspec:specify (or any other command)
...
- **task-317**: Convert jpspec commands to Copilot format (Priority: High, Labels: implement,copilot)
...
# Task 2: Convert jpspec commands to Copilot format
backlog task create "Convert jpspec commands to Copilot format" \
  -d "Convert all 15 jpspec.* commands from .claude/commands/jpspec/ to .github/agents/ with correct mode: frontmatter and resolved includes" \
  --ac "All 15 jpspec.* files exist in .github/agents/ with correct naming" \
  --ac "Each file has mode: jpspec.<name> frontmatter (not mode: agent)" \
...
backlog task create "Build sync-copilot-agents.sh automation script" \
  ...
  --ac "Script converts all 23 commands (jpspec + speckit) without manual edits" \
...
backlog task create "Test commands in VS Code and VS Code Insiders" \
  ...
  --ac "Test /jpspec:specify in VS Code - command appears in picker and executes correctly" \
  --ac "Test /jpspec:specify in VS Code Insiders - command appears and executes correctly" \
...
| Task 2: Convert jpspec | High | Medium | 2 hours |
...
- **Success Criteria**: 60%+ of jpspec command invocations come from Copilot Chat
...
- **Method**: Build minimal viable sync script for jpspec.specify only
- **Duration**: 2 days
...
- **Duration**: 1 week
- **Success Criteria**: All 5 users discover and use /jpspec:specify without documentation
...
1. All 23 commands (15 jpspec + 8 speckit) exist in `.github/agents/` with correct frontmatter
...
Then I see all 23 jpspec and speckit commands in the autocomplete list
And each command shows its description from frontmatter
```

**Scenario 2: Command Execution (jpspec:specify)**
```gherkin
Given I have a project with jpspec_workflow.yml configured
When I invoke /jpspec:specify with "Add user authentication feature"
...
```gherkin
Given I have modified .claude/commands/jpspec/specify.md
When I run ./scripts/bash/sync-copilot-agents.sh --dry-run
Then I see output: "Would update: .github/agents/jpspec.specify.md"
And no files are actually modified
```

**Scenario 4: Include Directive Resolution**
```gherkin
Given .claude/commands/jpspec/specify.md contains {{INCLUDE:_backlog-instructions.md}}
When I run the sync script
Then .github/agents/jpspec.specify.md contains the full content of _backlog-instructions.md
...
When I invoke /jpspec:implement with a task ID
...
- `jpspec_workflow.yml` for state transitions
...
- Convert only jpspec.* commands (skip speckit for MVP)
...
- **Definition**: Percentage of jpspec command invocations that occur via VS Code Copilot (vs CLI)
...
- **Definition**: Percentage of VS Code users who discover jpspec commands via Copilot Chat picker
- **Target**: 90%+ discovery rate within 1 week of feature announcement
- **Measurement**: User survey: "How did you first discover jpspec commands?" (options: Copilot picker, docs, word-of-mouth)

**Metric 2: First-Time Execution Success Rate**
- **Definition**: Percentage of users who successfully execute a jpspec command on first attempt
...
- **Measurement**: User survey: "How much time does in-IDE jpspec save you per day?"

**Metric 2: jpspec Adoption Rate**
- **Definition**: Percentage of VS Code users who use jpspec commands at least weekly
...
- **Definition**: Number of support requests related to "jpspec commands don't work in VS Code"
...
6. jpspec.validate
7. jpspec.operate
8. jpspec.prune-branch
9. jpspec.init
10. jpspec.reset
11. jpspec.security_fix
12. jpspec.security_report
13. jpspec.security_triage
14. jpspec.security_web
15. jpspec.security_workflow
...
**Claude Code Format** (`.claude/commands/jpspec/specify.md`):
```yaml
---
description: Create or update feature specifications using PM planner agent
mode: agent
...
**GitHub Copilot Format** (`.github/agents/jpspec.specify.md`):
```yaml
---
description: Create or update feature specifications using PM planner agent
mode: jpspec.specify
...
```markdown
{{INCLUDE:.claude/commands/jpspec/_workflow-base.md}}
```

**Relative Path Resolution**:
- Include paths are relative to `.claude/commands/jpspec/`
...
- [ ] Execute /jpspec:specify in VS Code - verify PRD creation
- [ ] Execute /jpspec:implement in VS Code - verify code generation
...
3. Run `/jpspec:plan` to create architecture design
4. Execute `/jpspec:implement` to begin development
