...
```
1. [Spec Creation]
   Developer: Run /jpspec:specify to create spec.md
   Output: spec.md with user stories

2. [Planning]
   Developer: Run /jpspec:plan to create plan.md
   Output: plan.md with architecture

3. [Task Generation] ← NEW INTEGRATION POINT
   Developer: Run /jpspec:tasks (modified to generate Backlog.md tasks)
   Output:
...
|------|------------|----------------|--------------|
| 1 | Run `/jpspec:tasks` | Tasks generated into Backlog.md | ✅ Delight: Automatic setup |
...
| Step | User Action | System Response | Pain/Delight |
|------|------------|----------------|--------------|
| 1 | Run `/jpspec:tasks` | Tasks generated, unassigned | ✅ Delight: Tasks ready for assignment |
...
- **When** I run `/jpspec:tasks` or `specify tasks generate`
...
- **Missing spec.md**: Error with guidance to run `/jpspec:specify` first
...
**Error Handling**:
- Missing spec.md: Error "Spec file not found. Run `/jpspec:specify` first."
...
**Implementation**:
- `/jpspec:tasks` accepts `--format` flag: `tasks-md` (default) or `backlog-md`
...
- **Example**: ❌ "Task generation failed" → ✅ "spec.md not found. Create it with: /jpspec:specify"
...
│                                                                 │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────┐  │
│  │ /jpspec:    │───▶│ Spec Files   │───▶│ Task Generator  │  │
│  │  specify    │    │              │    │                 │  │
...
```
1. User runs: /jpspec:tasks --feature 001-auth
...
**Edge Case Testing**:
- ❌ Missing spec.md → Error: "spec.md not found. Run /jpspec:specify first"
...
- Denominator: # of features generated via `/jpspec:tasks` in last 90 days
