...
This specification defines the schema and semantics for `jpspec_workflow.yml`, a YAML configuration file that describes:

1. **Task States** - Valid lifecycle states for development tasks
2. **Workflows** - Slash commands (`/jpspec:*`) that orchestrate agents
...
- **Workflow Orchestration**: Define how `/jpspec:*` commands execute
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `command` | string | Yes | Slash command (e.g., "/jpspec:specify") |
...
- Pattern: `/jpspec:<workflow_name>`
- Example: Workflow "specify" → command "/jpspec:specify"
...
3. **Command Pattern**: Commands must follow `/jpspec:<name>` pattern
...
|----------|---------|--------|--------------|--------------|
| specify | /jpspec:specify | product-requirements-manager | To Do | Specified |
| research | /jpspec:research | researcher, business-validator | Specified | Researched |
| plan | /jpspec:plan | software-architect, platform-engineer | Specified, Researched | Planned |
| implement | /jpspec:implement | frontend-engineer, backend-engineer, ai-ml-engineer, frontend-code-reviewer, backend-code-reviewer | Planned | In Implementation |
| validate | /jpspec:validate | quality-guardian, secure-by-design-engineer, tech-writer, release-manager | In Implementation | Validated |
| operate | /jpspec:operate | sre-agent | Validated | Deployed |
...
6. Create corresponding `/jpspec:<name>` command file
  ...
  "properties": {
    ...
    "workflows": {
      ...
      "additionalProperties": {
        ...
        "properties": {
          "command": { "type": "string", "pattern": "^/jpspec:" },
...
- [JPSpec Command Implementations](.claude/commands/jpspec/*.md)
