...
if [ ! -f "$TRIAGE_RESULTS" ]; then
    echo "❌ Error: triage-results.json not found. Run /jpspec:security triage first."
    exit 1
fi
```
...
**Generated by**: `/jpspec:security report`
**Report Version**: 1.0
...
**Command**: `/jpspec:security report [options]`
...
**Prerequisites**:
- Run `/jpspec:security scan` to generate scan-results.json
- Run `/jpspec:security triage` to generate triage-results.json
...
/jpspec:security report --format all

# Generate report without creating backlog tasks
/jpspec:security report --no-tasks
```
...
- `/jpspec:security triage` - Triage scan results
