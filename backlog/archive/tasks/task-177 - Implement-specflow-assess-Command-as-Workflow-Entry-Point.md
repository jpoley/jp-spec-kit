---
id: task-177
title: 'Implement /specflow:assess Command as Workflow Entry Point'
status: Done
assignee: []
...
<!-- AC:BEGIN -->
- [x] AC1: Create .claude/commands/specflow/assess.md slash command template
...
<!-- SECTION:NOTES:BEGIN -->
## Summary
Implement /specflow:assess as the mandatory entry point to the specflow workflow. This command evaluates complexity, risk, and architecture impact to recommend Full SDD, Spec-Light, or Skip SDD.

## Command Usage
```bash
/specflow:assess Build a user authentication system with OAuth2 and MFA
```
...
1. Run: `/specflow:specify {feature}`
2. Then: {subsequent commands based on mode}

## Override
To override this recommendation:
```bash
/specflow:specify --mode full {feature}
/specflow:specify --mode light {feature}
```
```
