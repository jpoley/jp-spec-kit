---
id: task-228
title: Create Claude Code hook to auto-emit events on /specflow completion
status: Done
assignee: []
...
Create a Claude Code PostToolUse hook that automatically emits jp-spec-kit events when /specflow slash commands complete. This provides automatic event emission without requiring explicit agent instructions.
...
**Architecture**:
```
Agent runs /specflow:implement
    ↓
Claude Code executes slash command
    ↓
PostToolUse hook fires
    ↓
Hook detects /specflow command completion
    ↓
...
**Implementation**:
1. Create `.claude/hooks/specflow-event-emitter.py`
...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 PostToolUse hook created at .claude/hooks/specflow-event-emitter.py
- [ ] #2 Hook detects /specflow:* command completion
