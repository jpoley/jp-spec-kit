...
**If `.specflow-light-mode` exists**, use light mode planning:
...
**For /specflow:plan**: Required input states are `workflow:Specified` OR `workflow:Researched`. Output state will be `workflow:Planned`.
...
4. **Update Workflow State**
   ...
   echo "  Next step: /specflow:implement"
   ```
