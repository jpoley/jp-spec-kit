...
This guide helps project architects and platform engineers choose the right technology stack based on project requirements, team composition, and business constraints. It should be used during the `/jpspec:plan` workflow to make informed architectural decisions.
...
## Usage in jpspec Workflow

This guide should be consulted during the `/jpspec:plan` workflow:

1. **Project Architect** uses this guide to recommend stack(s)
2. **Platform Engineer** validates technical feasibility
3. Both agents document the decision rationale in `/jpspec.constitution`
4. Stack choice influences the implementation approach in `/jpspec:implement`
