...
tools:
  ...
  - "Skill"
---
# /specflow:security report - Security Audit Report Generator
...
if [ ! -f "$SCAN_RESULTS" ]; then
    echo "❌ Error: scan-results.json not found. Run /specflow:security scan first to generate scan results."
    exit 1
fi

if [ ! -f "$TRIAGE_RESULTS" ]; then
    echo "❌ Error: triage-results.json not found. Run /specflow:security triage first."
    exit 1
fi
```
...
**Generated by**: `/specflow:security report`
**Report Version**: 1.0
...
**Command**: `/specflow:security report [options]`
...
**Prerequisites**:
- Run `/specflow:security scan` to generate scan-results.json
- Run `/specflow:security triage` to generate triage-results.json
...
/specflow:security report --format all

# Generate report without creating backlog tasks
/specflow:security report --no-tasks
```
...
- `/specflow:security triage` - Triage scan results
