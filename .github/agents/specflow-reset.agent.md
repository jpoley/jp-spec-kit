---
name: "specflow-reset"
description: "Reset workflow configuration by re-asking startup questions and regenerating specflow_workflow.yml"
target: "chat"
tools:
  - "Read"
  - "Write"
...
The `/specflow:reset` command:
1. Re-prompts for validation mode on each workflow transition
2. Regenerates `specflow_workflow.yml` with new settings
...
if [ ! -f "specflow_workflow.yml" ] && [ ! -d ".claude" ]; then
...
If no `specflow_workflow.yml` exists but `.claude/` directory does, this is a valid project that just needs workflow configuration.
...
Write this file to `./specflow_workflow.yml`.
...
📁 FILES UPDATED
  ...
  ✓ specflow_workflow.yml (regenerated)
  {✓ memory/constitution.md (if --constitution)}

🎯 NEXT STEPS

1. Review the generated specflow_workflow.yml
2. Commit the configuration:
   git add specflow_workflow.yml
   git commit -s -m "chore: reconfigure workflow validation modes"

💡 TIP: Run '/specflow:assess <feature>' to start a new workflow
```
...
This command mirrors the behavior of the CLI's internal `prompt_validation_modes()` and `generate_specflow_workflow_yml()` functions. Users can also reconfigure via CLI:
...
Before completing:
- [ ] specflow_workflow.yml exists and is valid YAML
