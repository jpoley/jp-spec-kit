...
**For /specflow:operate**: Required input state is `workflow:Validated`. Output state will be `workflow:Deployed`.

If the task doesn't have the required workflow state, inform the user:
- If task needs validation first: suggest running `/specflow:validate`
