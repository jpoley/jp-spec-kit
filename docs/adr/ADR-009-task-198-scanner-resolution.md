...
**Context:** Resolution of task-220 - Clarifying relationship between task-198 and /jpspec:security
**Supersedes:** None
**Amended by:** None
...
Task-220 was created to "Resolve Relationship with task-198 Unified Vulnerability Scanner" and determine integration strategy between `/jpspec:security` commands and a planned Trivy + Snyk scanner design.
...
3. **Native implementation supersedes** - The `/jpspec:security` architecture (ADR-005 through ADR-008) provides unified scanning through the adapter pattern
...
The native `/jpspec:security` implementation provides unified scanning through:
