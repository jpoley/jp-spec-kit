...
JP Spec Kit orchestrates complex software development workflows through `/specflow` commands (assess, specify, research, plan, implement, validate, operate). Currently, backlog.md tasks use simple statuses ("To Do", "In Progress", "Done") that don't reflect **which workflow phase** a task is in.
