...
Development teams using JP Spec Kit face a critical gap: **security vulnerabilities are discovered late in the development cycle**, when remediation costs are 10-30x higher than during active development. While the existing `/specflow:validate` command includes a `@secure-by-design-engineer` agent, there is no automated security scanning integrated into the workflow.
