# /specflow:security Architecture - Strategic Framing and System Design
...
Development teams using JP Specflow face a critical gap: **security vulnerabilities are discovered late in the development cycle**, when remediation costs are 10-30x higher than during active development. While the existing `/specflow:validate` command includes a `@secure-by-design-engineer` agent, there is no automated security scanning integrated into the workflow.
...
| **Architectural Fit** | Agent coordination mismatch | Native `/specflow` workflow | Native ✓ |
...
After:  Code → /specflow:security scan+triage+fix → Review patches → Apply → Rescan
```
...
│  ┌──────────────────────┐  ┌──────────────────────┐            │
│  │  /specflow:security    │  │  specify security    │            │
...
- CLI flags consistent with existing `/specflow` commands (`--format`, `--verbose`)
- State transitions integrate with specflow_workflow.yml
...
**Strengths:**
- Progressive disclosure: `/specflow:security scan` → triage → fix → audit
...
- [ ] `/specflow:security scan` command
- [ ] `/specflow:security triage` command
...
- [ ] `/specflow:security fix` command (AI patch generation)
...
**Mandate:** Projects MUST be able to use `/specflow` workflow without `/specflow:security`.
...
| `/specflow:security` adoption | >80% of projects | 6 weeks |
...
- **PRD:** `docs/prd/specflow-security-commands.md` - Product requirements
- **Assessment:** `docs/assess/specflow-security-commands-assessment.md` - Feature assessment
...
- task-215: Implement /specflow:security CLI Commands
