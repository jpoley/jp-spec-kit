# /specflow → /specflow Migration Execution Plan

**Date**: 2025-12-09
**Architecture**: Enterprise Software Architect
**Related**: docs/adr/ADR-specflow-to-specflow-migration-architecture.md

## Executive Summary

This document provides the operational execution plan for migrating 912+ occurrences of `/specflow` to `/specflow` across 150+ files. It complements the architectural decision record with ordered task dependencies, detailed file lists, and execution scripts.
...
sed -i 's|specflow_workflow.schema.json|specflow_workflow.schema.json|g' \
  memory/specflow_workflow.schema.json

# 1.3 Update command pattern in schema
sed -i 's|/specflow:|/specflow:|g' memory/specflow_workflow.schema.json

# 1.4 Rename workflow configs
git mv memory/specflow_workflow.yml memory/specflow_workflow.yml
git mv specflow_workflow.yml specflow_workflow.yml

# 1.5 Update command references in configs
sed -i 's|/specflow:|/specflow:|g' memory/specflow_workflow.yml
sed -i 's|/specflow:|/specflow:|g' specflow_workflow.yml
...
- memory/specflow_workflow.schema.json → specflow_workflow.schema.json
- memory/specflow_workflow.yml → specflow_workflow.yml
...
find templates/commands/specflow -type f -name "*.md" \
  -exec sed -i 's|/specflow:|/specflow:|g' {} +

# 2.3 Update include paths (self-references)
find templates/commands/specflow -type f -name "*.md" \
  -exec sed -i 's|commands/specflow/|commands/specflow/|g' {} +

# 2.4 Update workflow references
find templates/commands/specflow -type f -name "*.md" \
  -exec sed -i 's|specflow_workflow|specflow_workflow|g' {} +
...
**Files Changed**: 31 files (entire templates/commands/specflow/ directory)
...
- All .claude/commands/specflow/*.md deleted
...
cd .github/agents
git mv specflow-assess.agent.md specflow-assess.agent.md
git mv specflow-implement.agent.md specflow-implement.agent.md
git mv specflow-init.agent.md specflow-init.agent.md
git mv specflow-operate.agent.md specflow-operate.agent.md
git mv specflow-plan.agent.md specflow-plan.agent.md
git mv specflow-prune-branch.agent.md specflow-prune-branch.agent.md
git mv specflow-research.agent.md specflow-research.agent.md
git mv specflow-reset.agent.md specflow-reset.agent.md
git mv specflow-security_fix.agent.md specflow-security_fix.agent.md
git mv specflow-security_report.agent.md specflow-security_report.agent.md
git mv specflow-security_triage.agent.md specflow-security_triage.agent.md
git mv specflow-security_web.agent.md specflow-security_web.agent.md
git mv specflow-security_workflow.agent.md specflow-security_workflow.agent.md
git mv specflow-specify.agent.md specflow-specify.agent.md
git mv specflow-validate.agent.md specflow-validate.agent.md
cd ../..
...
- All specflow-*.agent.md renamed to specflow-*.agent.md
...
cd tests
git mv test_specflow_assess.py test_specflow_assess.py
git mv test_specflow_backlog_integration.py test_specflow_backlog_integration.py
git mv test_specflow_e2e.py test_specflow_e2e.py
git mv test_specflow_implement_backlog.py test_specflow_implement_backlog.py
git mv test_specflow_operate_backlog.py test_specflow_operate_backlog.py
git mv test_specflow_plan_backlog.py test_specflow_plan_backlog.py
git mv test_specflow_research_backlog.py test_specflow_research_backlog.py
git mv test_specflow_specify_backlog.py test_specflow_specify_backlog.py
git mv test_specflow_validate_backlog.py test_specflow_validate_backlog.py
git mv test_specflow_workflow_integration.py test_specflow_workflow_integration.py
cd ..
...
find tests -name "test_specflow_*.py" \
  -exec sed -i 's|test_specflow_|test_specflow_|g' {} +
...
- All test_specflow_*.py renamed to test_specflow_*.py
...
find src/specify_cli -name "*.py" \
  -exec sed -i 's|specflow_workflow\.yml|specflow_workflow.yml|g' {} +

find src/specify_cli -name "*.py" \
  -exec sed -i 's|specflow_workflow\.schema\.json|specflow_workflow.schema.json|g' {} +
...
git mv docs/guides/specflow-workflow-guide.md docs/guides/specflow-workflow-guide.md
git mv docs/guides/specflow-backlog-workflow.md docs/guides/specflow-backlog-workflow.md
git mv docs/reference/specflow-security-commands.md docs/reference/specflow-security-commands.md
git mv docs/architecture/specflow-security-architecture.md docs/architecture/specflow-security-architecture.md
git mv docs/platform/specflow-security-platform.md docs/platform/specflow-security-platform.md
git mv docs/assess/specflow-security-commands-assessment.md docs/assess/specflow-security-commands-assessment.md
git mv docs/prd/specflow-security-commands.md docs/prd/specflow-security-commands.md
git mv docs/diagrams/specflow-workflow.md docs/diagrams/specflow-workflow.md
git mv docs/diagrams/specflow-workflow.png docs/diagrams/specflow-workflow.png
git mv docs/diagrams/specflow-workflow.excalidraw docs/diagrams/specflow-workflow.excalidraw

# Additional doc files with specflow in name (check with find)
# git mv docs/specflow-workflow-spec.md docs/specflow-workflow-spec.md
# git mv docs/specflow-workflow-reference.md docs/specflow-workflow-reference.md
...
find docs -name "*.md" \
  -exec sed -i 's|specflow-workflow-guide\.md|specflow-workflow-guide.md|g' {} +

find docs -name "*.md" \
  -exec sed -i 's|specflow-backlog-workflow\.md|specflow-backlog-workflow.md|g' {} +

# 7.6 Update root files
sed -i 's|/specflow:|/specflow:|g' CLAUDE.md
sed -i 's|specflow_workflow|specflow_workflow|g' CLAUDE.md
sed -i 's|/specflow:|/specflow:|g' README.md
sed -i 's|/specflow:|/specflow:|g' CONTRIBUTING.md

# 7.7 Update table of contents
sed -i 's|specflow|specflow|g' docs/toc.yml

# Validation Checkpoint
# Check for broken links (manual or automated)
# grep -r "\[.*\](.*specflow.*)" docs/
```

**Files Changed**: 50+ files
- 16 files renamed (with specflow in filename)
...
- **Rename all /specflow commands to /specflow**
  - All slash commands: /specflow:* → /specflow:*
  - Configuration: specflow_workflow.yml → specflow_workflow.yml
  - Schema: specflow_workflow.schema.json → specflow_workflow.schema.json
  - Templates: templates/commands/specflow/ → specflow/
  - Agents: specflow-*.agent.md → specflow-*.agent.md
  - Tests: test_specflow_*.py → test_specflow_*.py
  - Affects 912+ occurrences across 150+ files

### Migration

See docs/guides/specflow-to-specflow-migration.md for migration guide.

EOF

# 9.8 Create migration guide
cat > docs/guides/specflow-to-specflow-migration.md <<EOF
...
\`\`\`bash
mv specflow_workflow.yml specflow_workflow.yml
\`\`\`
...
| /specflow:research | /specflow:research |
| /specflow:plan | /specflow:plan |
| /specflow:implement | /specflow:implement |
| /specflow:validate | /specflow:validate |
| /specflow:operate | /specflow:operate |
| /specflow:init | /specflow:init |
| /specflow:reset | /specflow:reset |
| /specflow:prune-branch | /specflow:prune-branch |
| /specflow:security_* | /specflow:security_* |
...
- docs/guides/specflow-to-specflow-migration.md (new file)
...
- ✅ No broken links
- ✅ No /specflow references remaining
...
# 7. Push to remote
git push origin migration-specflow-to-specflow-$(date +%Y%m%d)

# 8. Create PR
gh pr create --title "refactor: rename /specflow to /specflow across codebase" \
  --body "$(cat <<EOF
## Summary

Complete migration of /specflow branding to /specflow across entire codebase.
...
All \`/specflow\` commands are now \`/specflow\`:
- \`/specflow:assess\` → \`/specflow:assess\`
- \`/specflow:specify\` → \`/specflow:specify\`
- ... (all commands)
...
See: \`docs/guides/specflow-to-specflow-migration.md\`
...
- ✅ Coverage maintained
- ✅ No broken links
- ✅ No remaining /specflow references
...
|---------|-------------|-------------|-------|
| `/specflow:` | `/specflow:` | 750+ | All files |
| `specflow_workflow` | `specflow_workflow` | 100+ | Config, code, docs |
| `commands/specflow/` | `commands/specflow/` | 100+ | Include paths |
| `specflow-` | `specflow-` | 50+ | Agent filenames, refs |
| `test_specflow_` | `test_specflow_` | 10 | Test files |
| `specflow.` (events) | `specflow.` | 10+ | Python code |
