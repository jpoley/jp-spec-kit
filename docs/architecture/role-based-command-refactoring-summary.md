...
JP Spec Kit currently suffers from **command namespace overload**: 18+ commands in flat namespaces (`/specflow:*`, `/speckit:*`) create discoverability challenges and cognitive burden for users. Security commands are scattered, and there's no logical grouping by role or persona.
