...
This directory contains architectural design documents and decision records for the JP Flowspec project.
...
**Problem**: Three versions of flowspec commands exist (enhanced dev, minimal template, distributed), creating maintenance burden and inconsistent user experience.
...
**Decision**: Move enhanced flowspec commands from `.claude/commands/flowspec/` to `templates/commands/flowspec/` to establish single canonical source.
...
**Decision**: Use subdirectory structure (`flowspec/implement.md`) instead of flat structure with dots (`flowspec.implement.md`).
...
| task-271 | Migrate flowspec commands to templates | HIGH | architecture, migration |
| task-272 | Migrate speckit commands to subdirectory | HIGH | architecture, migration |
| task-273 | Update dev-setup command for flowspec symlinks | HIGH | cli, dev-setup, implementation |
...
- Move enhanced flowspec commands to templates
...
Commands organized in subdirectories by namespace (`flowspec/`, `speckit/`), not flat files with dots.
