...
This document analyzes the impact of the specflow → specflow migration on CI/CD pipelines, GitHub Actions workflows, and automated processes.
...
**Potential Impacts**:
- Workflow file itself may reference specflow in comments/documentation
- Test job runs `pytest tests/` - affected by test file renames
- No direct path references to specflow directories (good!)
...
**Verification**:
```bash
# Check workflow file for specflow references
grep -i "specflow" .github/workflows/ci.yml
...
**Potential Impacts**:
- DocFX configuration may reference specflow files
- Table of contents (toc.yml) may have specflow links
...
**Potential Impacts**:
- Release notes may reference specflow
- Changelog generation may include specflow references
...
**Potential Impacts**:
- HIGH IMPACT: `.github/agents/specflow-*.agent.md` files renamed
- Copilot may cache old agent names
- Agent references in workflows updated

**Required Changes**:
- ✅ Rename all `specflow-*.agent.md` to `specflow-*.agent.md` (handled by migration script)
- Update sync script if it hardcodes specflow paths
...
**Impact**: Medium - hooks may reference specflow paths.
...
**Impact**: Low - MCP configuration uses backlog server, not specflow-specific.

**Files to Check**:
- `.mcp.json` - should not reference specflow
- MCP server implementations

**Verification**:
```bash
# Check MCP config
cat .mcp.json | grep -i specflow
...
**Impact**: Low - most integrations use backlog.md, not specflow-specific endpoints.

**Integrations to Verify**:
- GitHub API (uses repo, not specflow paths)
...
**Impact**: High - extensive documentation references specflow.

**Affected Files**:
- `docs/guides/specflow-*.md` → `docs/guides/specflow-*.md`
- `docs/reference/specflow-*.md` → `docs/reference/specflow-*.md`
- `docs/diagrams/specflow-*.png` → `docs/diagrams/specflow-*.png`
...
**Verification**:
```bash
# Find all specflow references in docs
grep -r "specflow" docs/ --exclude-dir=_site
...
**Before**: `/specflow:assess`, `/specflow:specify`, etc.
...
- Consider keeping `/specflow:*` as aliases temporarily
- Update all documentation
- Communicate in release notes

#### 2. Workflow Configuration File

**Before**: `specflow_workflow.yml`
...
**Python API**:
- ✅ No breaking changes - imports use `specify_cli`, not specflow

**CLI Commands**:
- ✅ No breaking changes - CLI is `specify`, not specflow

**MCP Protocol**:
- ✅ No breaking changes - MCP servers use backlog, not specflow
...
1. **Pre-release** (Week before):
   ...
   - Tag current version as "last specflow version"
...
1. Keep `/specflow:*` as aliases:
   ```bash
   # Create symlinks
   ln -s specflow .claude/commands/specflow
   ```

2. Add deprecation warnings:
   ```python
   # In slash command handlers
   if command.startswith("/specflow:"):
       print("DEPRECATED: /specflow: commands renamed to /specflow:")
...
```yaml
# Add to CI workflow
- name: Verify no specflow references
  run: |
    # Allowed: CHANGELOG.md, migration docs
    JPSPEC_COUNT=$(grep -r "specflow" . \
      --exclude-dir=.git \
      --exclude-dir=.venv \
    ...
    if [ $JPSPEC_COUNT -gt 0 ]; then
      echo "ERROR: Found $JPSPEC_COUNT specflow references"
      grep -r "specflow" . --exclude-dir=.git --exclude-dir=.venv
      exit 1
    fi
```
...
## What Changed
- Slash commands: `/specflow:*` → `/specflow:*`
- Config file: `specflow_workflow.yml` → `specflow_workflow.yml`
- Agent files: `specflow-*.agent.md` → `specflow-*.agent.md`
...
- Migration Script: `scripts/bash/migrate-specflow-to-specflow.sh`
