...
| Metric | Before (Runtime Install) | After (Pre-baked Image) | Improvement |
|--------|-------------------------|-------------------------|-------------|
...
| Component | Image (Pre-baked) | Runtime (User Provides) | Rationale |
|-----------|------------------|------------------------|-----------|
...
**Repository**: `ai-coding-agents`
**Full image name**: `jpoley/specflow-agents`

**Alternative consideration**: Create organization `jpspeckit` → `jpspeckit/ai-coding-agents`
...
```json
{
  ...
  "mounts": [
    // Claude Code config (OAuth tokens)
    "source=${localEnv:HOME}${localEnv:USERPROFILE}/.claude,target=/home/vscode/.claude,type=bind",

    // GitHub Copilot config (OAuth tokens)
    "source=${localEnv:HOME}${localEnv:USERPROFILE}/.config/github-copilot,target=/home/vscode/.config/github-copilot,type=bind",
