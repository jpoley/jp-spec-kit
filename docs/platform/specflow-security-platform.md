# Platform Engineering Design: /specflow:security Commands
...
The `/specflow:security` feature is designed to maintain DORA Elite classification while introducing security quality gates:
...
CACHE_PATH=".specflow/cache/security/${CACHE_KEY}.json"
```
...
The `/specflow:security` commands integrate at three points in the development workflow:
...
graph LR
    ...
    H --> L[Developer Runs /specflow:security fix]
    L --> M[Apply Patches]
    M --> D
```
...
**Configuration:**
```yaml
# specflow_security.yml (project-level config)
security_gates:
  pre_commit:
    enabled: true
...
def ensure_semgrep_installed():
    ...
    cache_dir = Path.home() / ".cache" / "specflow" / "tools"
...
CACHE_SIZE=$(du -sh ~/.cache/specflow/tools | awk '{print $1}')
...
```mermaid
graph TD
    A[Code Commit] --> B[/specflow:security scan]
    B --> C{Parse Results}
    ...
    E --> I[/specflow:security triage]
    I --> J{AI: True Positive?}
    J -->|Yes| K[/specflow:security fix]
...
|--------|------|--------|---------|
| `specflow_security_scan_duration_seconds` | Histogram | tool, scan_type, outcome | Track scan performance |
| `specflow_security_findings_total` | Counter | severity, cwe_id, tool | Vulnerability trends |
| `specflow_security_triage_duration_seconds` | Histogram | model, finding_count | AI triage performance |
| `specflow_security_triage_accuracy` | Gauge | model | AI triage correctness |
| `specflow_security_fix_application_rate` | Gauge | severity | Fix adoption rate |
| `specflow_security_false_positive_rate` | Gauge | tool | Tool accuracy |
| `specflow_security_gate_blocks_total` | Counter | severity, gate_type | Pipeline impact |
...
avg_over_time(
  specflow_security_findings_total{severity="critical"}[7d]
)
```
...
histogram_quantile(0.95,
  sum(rate(specflow_security_scan_duration_seconds_bucket[5m])) by (le, tool)
)
```
...
avg_over_time(
  specflow_security_triage_accuracy[1h]
)
```
...
(
  specflow_security_fix_application_rate{severity="critical"}
  /
  specflow_security_findings_total{severity="critical"}
) * 100
```
...
groups:
  - name: specflow_security
    interval: 5m
    rules:
      - alert: CriticalVulnerabilityFound
        expr: specflow_security_findings_total{severity="critical"} > 0
        for: 5m
        annotations:
      ...
      - alert: SecurityScanPerformanceDegraded
        expr: |
          histogram_quantile(0.95,
            sum(rate(specflow_security_scan_duration_seconds_bucket[10m])) by (le, tool)
          ) > 300
        for: 15m
        annotations:
      ...
      - alert: AITriageAccuracyLow
        expr: specflow_security_triage_accuracy < 0.8
        for: 1h
        annotations:
...
**Usage Documentation**:
```markdown
# Using /specflow:security in CI/CD
...
```bash
# Build
docker build -t specflowkit/security-scanner:latest -f docker/security-scanner.Dockerfile .

# Publish to GHCR
docker tag specflowkit/security-scanner:latest ghcr.io/yourusername/specflowkit-security-scanner:1.0.0
docker push ghcr.io/yourusername/specflowkit-security-scanner:1.0.0
```
...
docker run --rm -v $(pwd):/src \
  ghcr.io/yourusername/specflowkit-security-scanner:1.0.0 \
...
backlog task create "Setup CI/CD Security Scanning Pipeline" \
  -d "Configure GitHub Actions for security scanning integration with /specflow:security commands" \
...
backlog task create "Implement Security Scanning Observability" \
  -d "Add Prometheus metrics, structured logging, and dashboards for /specflow:security" \
...
backlog task create "Create Pre-commit Hook Configuration for Security Scanning" \
  ...
  --ac "Create .pre-commit-config.yaml template with specflow-security hook" \
...
backlog task create "Implement Security Policy as Code Configuration" \
  -d "Build .specflow/security-policy.yml parser and enforcement engine" \
...
backlog task create "Build and Publish Security Scanner Docker Image" \
  ...
  --ac "Publish to GHCR (ghcr.io/yourusername/specflowkit-security-scanner)" \
