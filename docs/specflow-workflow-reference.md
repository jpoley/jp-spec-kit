...
Building production-ready software requires expertise across multiple domains: product management, architecture, frontend, backend, security, testing, operations, and more. specflow brings together specialized AI agents that work together seamlessly, enabling you to:
...
specflow automates the software delivery lifecycle through six specialized workflows:
...
/specflow:research Build a task management REST API

# 2. Create product specifications
/specflow:specify Task management REST API with authentication

# 3. Design architecture and platform
/specflow:plan Task management REST API

# 4. Implement the backend
/specflow:implement Task management REST API backend

# 5. Validate and prepare for release
/specflow:validate Task management REST API

# 6. (Optional) Set up operations infrastructure
/specflow:operate Task management REST API
```
...
**Command:**
```bash
/specflow:research AI-powered customer service chatbot with sentiment analysis and multi-language support
```
...
**Command:**
```bash
/specflow:specify AI customer service chatbot based on research findings
```
...
**Command:**
```bash
/specflow:plan AI customer service chatbot
```
...
**Command:**
```bash
/specflow:implement AI customer service chatbot
```
...
Ready for /specflow:validate phase
```
...
**Command:**
```bash
/specflow:validate AI customer service chatbot
```
...
**Command:**
```bash
/specflow:operate AI customer service chatbot
```
...
**Input Format:**
```bash
/specflow:research <project description>
```

**Example:**
```bash
/specflow:research AI-powered code review tool for enterprise development teams
```
...
**Input Format:**
```bash
/specflow:specify <feature description> [context from research]
```

**Example:**
```bash
/specflow:specify Authentication system with OAuth2, SSO, and MFA based on enterprise security requirements
```
...
**Input Format:**
```bash
/specflow:plan <project name> [PRD and requirements]
```

**Example:**
```bash
/specflow:plan E-commerce checkout system with payment processing and inventory management
```
...
**Input Format:**
```bash
/specflow:implement <feature description> [architecture and specs]
```

**Example:**
```bash
/specflow:implement Real-time notification system with WebSocket support and push notifications
```
...
**Input Format:**
```bash
/specflow:validate <feature name> [code and artifacts]
```

**Example:**
```bash
/specflow:validate Payment processing API with PCI DSS compliance requirements
```
...
**Input Format:**
```bash
/specflow:operate <project name> [architecture and platform specs]
```

**Example:**
```bash
/specflow:operate Microservices platform with multi-region deployment and 99.99% SLA
```
...
specflow uses a sophisticated multi-agent orchestration system that coordinates specialized AI agents across the software delivery lifecycle.
...
```bash
/specflow:research <idea>      # Start here
/specflow:specify <idea>       # Then specify
/specflow:plan <idea>          # Then plan
/specflow:implement <idea>     # Then implement
/specflow:validate <idea>      # Then validate
/specflow:operate <idea>       # Finally operate
```
...
```bash
/specflow:specify Real-time collaborative document editor with operational transformation,
...
```bash
/specflow:specify Document editor
```
...
```bash
/specflow:research Blockchain-based supply chain tracking for pharmaceutical industry
...
**Do:** Carefully review the PRD from `/specflow:specify`
...
**Do:** Preserve ADRs from `/specflow:plan` in your repository
```bash
...
/specflow:implement Background job processing system

# For prototypes, skip operations
/specflow:research → specify → plan → implement
...
**Note:** specflow automatically runs agents in parallel when possible
- `/specflow:plan`: Architecture + Platform (parallel)
- `/specflow:implement`: Frontend + Backend + ML (parallel)
- `/specflow:validate`: QA + Security (parallel)
...
```bash
# Instead of:
/specflow:specify payment system

# Provide more context:
/specflow:specify payment processing system supporting credit cards, ACH, and crypto,
...
**Root Cause:**
- Implementation didn't follow architecture from `/specflow:plan`
- Requirements from `/specflow:specify` not met

**Solution:**
1. Review architecture and requirements
2. Re-run `/specflow:implement` with explicit architecture constraints
...
**Symptoms:**
- `/specflow:validate` blocks release
...
3. **Re-run validation**: After fixes, run `/specflow:validate` again
...
4. Re-run `/specflow:validate` with updated code
...
1. Provide tech-writer agent with more context:
   ```bash
   /specflow:validate <feature> [include detailed API specs, architecture diagrams, test results]
   ```
...
1. **Specify technology choices in architecture phase:**
   ```bash
   /specflow:plan E-commerce platform using Go backend, React frontend, PostgreSQL database
   ```
...
3. **Explicitly constrain in implementation:**
   ```bash
   /specflow:implement API service in Go using Chi router and PostgreSQL with sqlx
   ```
...
specflow transforms software delivery by orchestrating specialized AI agents across the complete lifecycle:
