...
This guide provides step-by-step instructions for executing the specflow → specflow migration. The migration affects 900+ occurrences across 100+ files and requires approximately 3 hours to complete with full verification.

## Quick Start

```bash
# 1. Preview the migration
./scripts/bash/migrate-specflow-to-specflow.sh --dry-run --verbose
...
ls -la scripts/bash/migrate-specflow-to-specflow.sh
...
**Expected Output**:
```
[INFO] Starting specflow → specflow migration
...
[INFO] DRY-RUN: Would rename: templates/commands/specflow -> templates/commands/specflow
[INFO] DRY-RUN: Would rename: .claude/commands/specflow -> .claude/commands/specflow
[INFO] Renaming files...
...
./scripts/bash/migrate-specflow-to-specflow.sh --verbose

# OR execute without prompts (for automation)
./scripts/bash/migrate-specflow-to-specflow.sh --force --verbose
```
...
2. ✅ No remaining specflow references (except allowed locations)
...
[✓] No remaining specflow references found
[✓] Directory exists: templates/commands/specflow
[✓] Old directory removed: templates/commands/specflow
...
...
# 4. Create commit
git commit -m "Rename /specflow to /specflow

- Rename slash commands (/specflow:* → /specflow:*)
- Rename directories (templates/commands/specflow → specflow)
- Rename workflow config (specflow_workflow.yml → specflow_workflow.yml)
- Rename test files (test_specflow_* → test_specflow_*)
- Rename agent files (specflow-*.agent.md → specflow-*.agent.md)
- Update all documentation references
- Verify all tests pass
- Update CI/CD workflows

Migration executed by scripts/bash/migrate-specflow-to-specflow.sh
...
### Changed
- **BREAKING**: Renamed all /specflow: commands to /specflow:
...
- Renamed `specflow_workflow.yml` to `specflow_workflow.yml`
...
# 2. Update README.md
# - Change specflow references to specflow
...
# Verify restoration
grep -r "specflow" . --exclude-dir=.git | wc -l  # Should be high
...
**Fix**:
```bash
chmod +x scripts/bash/migrate-specflow-to-specflow.sh
```
...
**Fix**:
```bash
# 1. Check for remaining specflow references
grep -r "specflow" tests/

# 2. Fix imports manually
# Example: from specify_cli.specflow_* → specify_cli.specflow_*
...
# 2. Update manually
sed -i 's|specflow|specflow|g' docs/problematic-file.md
...
- **Migration Plan**: `docs/adr/specflow-to-specflow-migration-plan.md`
