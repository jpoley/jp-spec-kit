...
This diagram visualizes the complete `/specflow` command structure, showing all slash commands and their sub-agent invocations with execution patterns.
...
flowchart TD
    ...
    %% Commands
    CMD_SPECIFY["/specflow:specify<br/>Create/Update Feature Specs"]
    CMD_PLAN["/specflow:plan<br/>Execute Planning Workflow"]
    CMD_RESEARCH["/specflow:research<br/>Research & Business Validation"]
    CMD_IMPLEMENT["/specflow:implement<br/>Feature Implementation"]
    CMD_VALIDATE["/specflow:validate<br/>QA & Validation"]
    CMD_OPERATE["/specflow:operate<br/>Operations & SRE"]
...
| Element | Description |
|---------|-------------|
| 🔵 **Blue Rectangles** | `/specflow` Slash Commands (entry points) |
...
- **/specflow:plan**: Software Architect + Platform Engineer (2 agents in parallel)
- **/specflow:implement Phase 1**: Frontend + Backend + AI/ML Engineers (up to 3 agents in parallel)
- **/specflow:validate Phase 1**: QA + Security (2 agents in parallel)

#### Sequential Execution
Agents execute one after another, with each depending on the previous:
- **/specflow:research**: Researcher → Business Validator (2 agents sequential)
- **/specflow:implement Phase 2**: Code reviews after implementation (sequential)
- **/specflow:validate Phases**: Testing → Documentation → Release Management (sequential)

#### Single Agent
Commands with one specialized agent:
- **/specflow:specify**: Product Requirements Manager (1 agent)
- **/specflow:operate**: Site Reliability Engineer (1 agent)
...
|---------|--------|-------------------|
| /specflow:specify | 1 | Single |
| /specflow:plan | 2 | Parallel |
| /specflow:research | 2 | Sequential |
| /specflow:implement | 3-5 | Multi-phase (parallel + sequential) |
| /specflow:validate | 4 | Multi-phase (parallel + sequential + human gate) |
| /specflow:operate | 1 | Single |
...
1. **Specification**: `/specflow:specify` → Product Requirements Manager creates PRD
2. **Planning**: `/specflow:plan` → Architect + Platform Engineer design system (parallel)
3. **Research** (optional): `/specflow:research` → Validate market fit and feasibility
4. **Implementation**: `/specflow:implement` → Engineers build + reviewers validate
5. **Validation**: `/specflow:validate` → QA + Security + Docs + Release approval
6. **Operations**: `/specflow:operate` → SRE establishes operational infrastructure

### Constitutional Development

The `/specflow:plan` command builds out `/speckit.constitution` with:
...
- [JP Spec Workflow Reference](../specflow-workflow-reference.md)
