# PRD: /specflow:security Commands - Comprehensive Security Scanning for SDD Workflow

**Feature**: specflow-security-commands
**Date**: 2025-12-02
...
Introduce `/specflow:security` commands that seamlessly integrate security scanning into the SDD workflow through intelligent tool orchestration, AI-assisted triage, and automated fix suggestions.

**Core Commands:**
```bash
/specflow:security scan      # Orchestrate Semgrep (+ optional CodeQL) scans
/specflow:security audit     # Generate comprehensive security audit reports
/specflow:security triage    # AI-assisted vulnerability prioritization and triage
/specflow:security fix       # Generate fix suggestions with code patches
```
...
| **Leading Indicators** | | | |
| `/specflow:security scan` adoption rate | >80% of projects | 6 weeks | Projects using command / Total projects |
...
```mermaid
graph LR
    A[Completes feature implementation] --> B[Runs /specflow:security scan]
    B --> C[Semgrep finds 5 issues]
    C --> D[Runs /specflow:security triage]
    D --> E[AI ranks: 2 critical, 3 false positives]
    E --> F[Runs /specflow:security fix for critical issues]
...
```mermaid
graph LR
    A[Configures /specflow:security in CI] --> B[Teams use /specflow:implement]
    B --> C[Auto-triggers /specflow:security scan]
    C --> D[Blocks PR if critical issues found]
    D --> E[Developers run /specflow:security fix]
...
**Key Moments:**
- **Configuration**: One-time setup in `specflow_workflow.yml` enables enforcement for all teams
...
**Acceptance Criteria:**
- [ ] AC1.1: Running `/specflow:security scan` executes Semgrep with OWASP ruleset by default
...
**Acceptance Criteria:**
- [ ] AC2.1: `/specflow:security triage` reads scan results from previous scan
...
**Acceptance Criteria:**
- [ ] AC3.1: `/specflow:security fix` generates patches for triaged True Positive findings
...
**Acceptance Criteria:**
- [ ] AC4.1: `/specflow:security audit` generates report using template `templates/security-report-template.md`
...
**Acceptance Criteria:**
- [ ] AC5.1: `/specflow:security scan` can run in CI environment without interactive prompts
...
- **Scenario**: User runs `/specflow:security scan --tool codeql` on private repository without CodeQL license
...
- **Scenario**: User runs `/specflow:security triage` without running `scan` first
- **Expected Behavior**: Command displays helpful error and suggests running scan
- **Error Message**: "❌ No scan results found. Run `/specflow:security scan` first to generate findings."
...
**Question**: Will developers and security engineers actually use `/specflow:security` commands?

**Risk Level**: Medium
**Confidence**: 60% (needs validation)

**Evidence For:**
- ✅ Existing `/specflow:validate` is actively used (50+ executions in beta)
...
**Evidence For:**
- ✅ Command structure follows existing `/specflow:*` conventions (low learning curve)
...
3. **Guided Workflows**: Command suggests next steps (e.g., "Run `/specflow:security triage` to prioritize findings")
...
### FR1: Security Scanner Orchestration (`/specflow:security scan`)
...
# Specify scanner
/specflow:security scan --tool semgrep
/specflow:security scan --tool codeql
/specflow:security scan --tool all

# Custom ruleset
/specflow:security scan --config custom-rules.yml

# Incremental scan (only changed files)
/specflow:security scan --incremental

# Fail on severity threshold
/specflow:security scan --fail-on critical,high

# Output format
/specflow:security scan --format json|markdown|sarif

# Verbose output
/specflow:security scan --verbose
```
...
### FR2: AI-Powered Vulnerability Triage (`/specflow:security triage`)
...
/specflow:security triage --interactive

# Specify scan results file
/specflow:security triage --scan-results path/to/results.json

# Filter by severity
/specflow:security triage --min-severity high

# Output format
/specflow:security triage --format json|markdown
```
...
### FR3: Automated Fix Generation (`/specflow:security fix`)
...
### FR4: Security Audit Report Generation (`/specflow:security audit`)
...
/specflow:security audit --format markdown|html|pdf

# Include only specific severities
/specflow:security audit --severity critical,high

# Compliance mode (extra detail for auditors)
/specflow:security audit --compliance soc2|iso27001|hipaa
```
...
**Description**: Integrate `/specflow:security` commands into existing SDD workflow.

**Sub-Requirements:**

#### FR5.1: Workflow State Transitions
Potential integration points:
1. **Option A: Extend `/specflow:validate`** - Security scan becomes sub-phase of validation
...
Example:
```bash
/specflow:security triage --create-tasks
```
...
- [ ] Verify with `/specflow:security scan` (finding FINDING-001 resolved)
...
jobs:
  security:
    ...
    steps:
      ...
      - name: Security Scan
        run: /specflow:security scan --fail-on critical,high
      - name: Upload Results
        if: always()
...
Create `.specflow/security-config.yml`:
...
- Users can provide custom Semgrep rules in `.specflow/security-rules/`
...
- Example:
  ...
  Or allow automatic installation:
    /specflow:security scan --auto-install
  ```
...
- **Strategy**: Download-on-demand with caching
  - First run: Download Semgrep (~50MB), cache in `~/.specflow/tools/`
...
- **Command Reference**: `docs/reference/specflow-security-commands.md`
  - All command flags
...
| **task-215** | Implement /specflow:security CLI Commands | High | security, implement, cli |
| **task-216** | Integrate /specflow:security with Workflow and Backlog | High | security, implement, workflow |
...
5. **task-216: Workflow Integration** - Wire into specflow ecosystem
...
6. **task-217: Configuration System** - User customization
   - `.specflow/security-config.yml` support
...
2. Run `/specflow:security scan` on each
...
Before the `/specflow:security` commands can be considered "Done," the following criteria must be met:

#### AC1: Core Functionality
- [ ] `/specflow:security scan` successfully scans Python, JavaScript, Go, and TypeScript projects
- [ ] Scan results include severity, CWE ID, OWASP category, file location, and code snippet
- [ ] `/specflow:security triage` classifies findings with >85% accuracy (validated against benchmark)
- [ ] `/specflow:security fix` generates syntactically valid patches for 80%+ of findings
- [ ] `/specflow:security audit` generates complete report using security-report-template.md
...
#### AC4: Integration
- [ ] Commands integrate with specflow_workflow.yml (documented integration options)
...
- [ ] Command Reference published at `docs/reference/specflow-security-commands.md`
...
1. **Semgrep Integration**:
   ...
   - Test custom ruleset loading from `.specflow/security-rules/`
...
1. **Happy Path: Solo Developer Workflow**:
   - User runs `/specflow:security scan` on new Python project
   - Scan finds 5 vulnerabilities (3 TP, 2 FP)
   - User runs `/specflow:security triage`
   - AI correctly classifies 3 as TP, 2 as FP
   - User runs `/specflow:security fix` for 3 TP findings
   - User applies patches and re-scans
   - Scan now shows 0 vulnerabilities
   - User runs `/specflow:security audit` to generate report

2. **CI/CD Integration Workflow**:
   - GitHub Actions workflow triggers on PR
   - Workflow runs `/specflow:security scan --fail-on critical,high`
...
3. **Error Recovery Workflow**:
   - User runs `/specflow:security triage` without prior scan
...
- **Network dependency**: Cache Semgrep Registry rules in `~/.specflow/cache/semgrep-rules/`
...
**Dependency**: `specflow_workflow.yml` schema must support security workflow states

**Requirement**: Either extend existing `/specflow:validate` or add new state "Security Audited"

**Decision Point**: Resolved in task-220 (determine if `/specflow:security` is standalone or part of validate)
...
**Background**: task-198 ("Design Unified Vulnerability Scanner - Trivy + Snyk Integration") is currently In Progress and overlaps with `/specflow:security` scope.
...
- `/specflow:security scan` becomes the unified entry point
...
- `/specflow:security scan` → Code vulnerabilities (SAST)
...
- Create `/specflow:security scan --all` to invoke both Semgrep and Trivy/Snyk
...
1. Track vulnerabilities found by `/specflow:security scan` in development
...
**Data Source**: Telemetry from `/specflow:security` commands (with user consent)

### Leading Indicators (Predict North Star Success)

#### LI1: `/specflow:security scan` Adoption Rate

**Definition**: Percentage of JP Spec Kit projects that have run `/specflow:security scan` at least once
...
**Data Source**: User feedback via `/specflow:security triage --interactive` or manual FP reports
...
**Definition**: "How likely are you to recommend `/specflow:security` commands to a colleague?" (0-10 scale)
...
**Data Source**: In-app survey after `/specflow:security triage` command
...
- AI triage feedback: After first `/specflow:security triage` run
- Fix quality: After first `/specflow:security fix` run
...
| **task-215** | Implement /specflow:security CLI Commands | User interface |
| **task-216** | Integrate /specflow:security with Workflow and Backlog | Workflow integration |
...
The `/specflow:security` feature uses a three-layer architecture to support both interactive AI-assisted workflows and automated CI/CD pipelines.

### Architecture Diagram

```
┌─────────────────────────────────────────────────────────────┐
│  /specflow:security (Slash Commands)                         │
│  ─────────────────────────────────────────────────────────  │
...
This section documents the relationship between JP Spec Kit's `/specflow:security` and the [Raptor framework](https://github.com/gadievron/raptor).
...
| Capability | Raptor | specflow:security | Status |
|------------|--------|-----------------|--------|
| **Static Analysis** | | | |
...
**Status**: ✅ Complete - Ready for Planning Phase (`/specflow:plan`)
...
5. Proceed to `/specflow:plan` for architecture and platform design
