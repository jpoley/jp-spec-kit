# Feature Assessment: /specflow:security Commands with Raptor Integration
...
Add comprehensive security scanning capabilities to JP Specflow via new `/specflow:security` commands with subcommands. The decision point is whether to:
...
- `/specflow:validate` - Includes security validation agent
...
| Integration Points | 7/10 | Semgrep, CodeQL, AFL++ (optional), existing specflow workflows, backlog.md, CI/CD pipelines |
| **Average** | **7.3/10** | |
...
| Breaking Changes | 5/10 | New commands, no breaking changes to existing specflow commands |
...
### Option B: Build Native `/specflow:security`

**Pros:**
- Tight integration with existing specflow workflow
...
## Proposed `/specflow:security` Command Structure

```bash
# Subcommands
/specflow:security scan      # Run security scans (Semgrep, optionally CodeQL)
/specflow:security audit     # Generate security audit report
/specflow:security triage    # AI-assisted vulnerability triage
/specflow:security fix       # Generate fix suggestions for findings

# Future (post-MVP)
/specflow:security fuzz      # Binary fuzzing (AFL++)
/specflow:security pentest   # Penetration testing mode
```

### MVP Scope (Recommended for First Implementation)

1. `/specflow:security scan` - Orchestrate Semgrep with custom rules
2. `/specflow:security audit` - Generate formatted security report
3. `/specflow:security triage` - AI triage of findings
...
```bash
/specflow:specify specflow-security-commands
```
...
1. Create `docs/prd/specflow-security-commands-spec.md` with:
   - Problem statement
...
/specflow:assess specflow-security-commands --mode full

# Force spec-light mode
/specflow:assess specflow-security-commands --mode light

# Force skip SDD
/specflow:assess specflow-security-commands --mode skip
```
