# Feature Assessment: /jpspec:security Commands with Raptor Integration
...
- `/jpspec:validate` - Includes security validation agent
...
| Integration Points | 7/10 | Semgrep, CodeQL, AFL++ (optional), existing jpspec workflows, backlog.md, CI/CD pipelines |
| **Average** | **7.3/10** | |
...
| Breaking Changes | 5/10 | New commands, no breaking changes to existing jpspec commands |
...
### Option B: Build Native `/jpspec:security`

**Pros:**
- Tight integration with existing jpspec workflow
...
## Proposed `/jpspec:security` Command Structure

```bash
# Subcommands
/jpspec:security scan      # Run security scans (Semgrep, optionally CodeQL)
/jpspec:security audit     # Generate security audit report
/jpspec:security triage    # AI-assisted vulnerability triage
/jpspec:security fix       # Generate fix suggestions for findings

# Future (post-MVP)
/jpspec:security fuzz      # Binary fuzzing (AFL++)
/jpspec:security pentest   # Penetration testing mode
```

### MVP Scope (Recommended for First Implementation)

1. `/jpspec:security scan` - Orchestrate Semgrep with custom rules
2. `/jpspec:security audit` - Generate formatted security report
3. `/jpspec:security triage` - AI triage of findings
...
```bash
/jpspec:specify jpspec-security-commands
```
...
1. Create `docs/prd/jpspec-security-commands-spec.md` with:
   - Problem statement
...
/jpspec:assess jpspec-security-commands --mode full

# Force spec-light mode
/jpspec:assess jpspec-security-commands --mode light

# Force skip SDD
/jpspec:assess jpspec-security-commands --mode skip
```
