...
| Component Count | 6/10 | New components: event model, hook definitions, hook runner. Modified: CLI layer, all /specflow commands, workflow state management |
| Integration Points | 5/10 | Must integrate with: existing /specflow commands, backlog.md, file system, potential future webhooks/CI |
| **Average** | **6.0/10** | |
...
| Dependencies Affected | 6/10 | All /specflow commands need to emit events; affects workflow engine, CLI, and enables future multi-agent orchestration |
| **Average** | **5.7/10** | |
...
/specflow:assess agent-hooks --mode light
```
...
```bash
/specflow:assess agent-hooks --mode skip
```
