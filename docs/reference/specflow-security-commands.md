# /specflow:security Commands Reference

Complete reference for `/specflow:security` slash commands used in Claude Code sessions.

## Command Overview

The `/specflow:security` command suite provides AI-powered security scanning, triage, remediation, and reporting integrated into your Claude Code workflow.

### Available Commands

| Command | Purpose | Input Required | Output Generated |
|---------|---------|----------------|------------------|
| `/specflow:security scan` | Discover vulnerabilities | None | scan-results.json |
| `/specflow:security triage` | Classify findings with AI | scan-results.json | triage-report.md |
| `/specflow:security fix` | Generate security patches | triage-results.json | patches/, fix-summary.md |
| `/specflow:security report` | Generate audit report | scan & triage results | audit-report.md |
