...
This specification defines the schema and semantics for `specflow_workflow.yml`, a YAML configuration file that describes:

1. **Task States** - Valid lifecycle states for development tasks
2. **Workflows** - Slash commands (`/specflow:*`) that orchestrate agents
...
- **Workflow Orchestration**: Define how `/specflow:*` commands execute
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `command` | string | Yes | Slash command (e.g., "/specflow:specify") |
...
- Pattern: `/specflow:<workflow_name>`
- Example: Workflow "specify" → command "/specflow:specify"
...
3. **Command Pattern**: Commands must follow `/specflow:<name>` pattern
...
|----------|---------|--------|--------------|--------------|
| specify | /specflow:specify | product-requirements-manager | To Do | Specified |
| research | /specflow:research | researcher, business-validator | Specified | Researched |
| plan | /specflow:plan | software-architect, platform-engineer | Specified, Researched | Planned |
| implement | /specflow:implement | frontend-engineer, backend-engineer, ai-ml-engineer, frontend-code-reviewer, backend-code-reviewer | Planned | In Implementation |
| validate | /specflow:validate | quality-guardian, secure-by-design-engineer, tech-writer, release-manager | In Implementation | Validated |
| operate | /specflow:operate | sre-agent | Validated | Deployed |
...
6. Create corresponding `/specflow:<name>` command file
  ...
  "properties": {
    ...
    "workflows": {
      ...
      "additionalProperties": {
        ...
        "properties": {
          "command": { "type": "string", "pattern": "^/specflow:" },
...
```
project/
├── specflow_workflow.yml    # <-- This file
├── .specify/
├── .claude/
│   └── commands/
│       └── specflow/
...
def load_workflow_config(path: Path = Path("specflow_workflow.yml")) -> dict:
...
- [JPSpec Command Implementations](.claude/commands/specflow/*.md)
