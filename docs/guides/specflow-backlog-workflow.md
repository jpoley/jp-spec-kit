...
Complete guide to using `/specflow` commands with backlog.md task management for end-to-end spec-driven development.

## Table of Contents

- [Overview](#overview)
- [How It Works](#how-it-works)
- [Task Format for JPSpec Compatibility](#task-format-for-specflow-compatibility)
...
The `/specflow` workflow system and backlog.md are deeply integrated to provide:

1. **State-Driven Commands**: Each `/specflow` command validates task state before execution
...
```
/specflow:assess    →  Evaluate complexity (To Do → Assessed)
       ↓
/specflow:specify   →  Create PRD + tasks (Assessed → Specified)
       ↓
/specflow:research  →  Market validation (Specified → Researched)
       ↓
/specflow:plan      →  Architecture design (Researched → Planned)
       ↓
/specflow:implement →  Code development (Planned → In Implementation)
       ↓
/specflow:validate  →  QA + security (In Implementation → Validated)
       ↓
/specflow:operate   →  Deploy to production (Validated → Deployed)
```
...
**Design commands** (`/specflow:specify`, `/specflow:research`, `/specflow:plan`) create new tasks:

```bash
# /specflow:specify creates implementation tasks
...
# /specflow:plan creates architecture tasks
...
**Implementation commands** (`/specflow:implement`, `/specflow:validate`, `/specflow:operate`) work from existing tasks:
...
Before executing, each `/specflow` command validates task state:

```bash
# This will ERROR if task is in wrong state
/specflow:implement  # Requires task in "Planned" state

# Error message:
# Cannot run /specflow:implement
#   Current state: "To Do"
#   Required states: Planned
# Suggestions:
#   - Run /specflow:plan first
```
...
Tasks must include these fields for full `/specflow` integration:
...
| `Assessed` | Complexity evaluated | After `/specflow:assess` |
| `Specified` | PRD complete | After `/specflow:specify` |
| `Researched` | Research validated | After `/specflow:research` |
| `Planned` | Architecture designed | After `/specflow:plan` |
| `In Progress` | Engineer working | During `/specflow:implement` |
| `In Implementation` | Active coding | `/specflow:implement` phase |
| `Validated` | QA + security passed | After `/specflow:validate` |
| `Deployed` | In production | After `/specflow:operate` |
...
Specified ─────────────────────┐                      │
   │                           │                       │
   ↓ /specflow:research          │ (skip research)       │
Researched                     │                       │
   │                           │                       │
   └───────────┬───────────────┘                       │
               ↓ /specflow:plan                          │
...
To Do → Done  # Skip all /specflow commands
...
/specflow:assess <feature description>
...
/specflow:implement User authentication  # ERROR: No tasks found
...
**Problem**: `/specflow:implement` can't find tasks to work on.
...
### "Cannot run /specflow:X - wrong state" Error
...
# Check .claude/commands/specflow/*.md files
```
...
- **[JP Spec Workflow Guide](specflow-workflow-guide.md)** - Complete workflow documentation
