# JP Spec Kit /specflow Workflow Guide

Comprehensive guide to using the /specflow command orchestration system for specification-driven development with AI-powered agents.

## Table of Contents

- [Executive Summary](#executive-summary)
- [Overview](#overview)
- [The Seven /specflow Commands](#the-seven-specflow-commands)
...
- [Diagram: /specflow Workflow](#diagram-specflow-workflow)
- [References](#references)

## Executive Summary

### What is the /specflow Workflow System?

The /specflow workflow system is JP Spec Kit's **AI-powered command orchestration layer** that coordinates specialized agents through the complete software development lifecycle. It transforms specification-driven development from a manual process into an automated, traceable workflow.
...
```
/specflow:assess    → Evaluate feature complexity (Simple/Medium/Complex)
/specflow:specify   → Create comprehensive Product Requirements Document (PRD)
/specflow:research  → Market research + business validation
/specflow:plan      → Architecture + platform design (parallel agents)
/specflow:implement → Frontend + backend implementation with code review
/specflow:validate  → QA testing + security + docs + release readiness
/specflow:operate   → CI/CD pipelines + Kubernetes + observability
```

### When to Use /specflow

**Use Full Workflow For**:
- Complex features (21-32 complexity score from `/specflow:assess`)
...
See [/specflow:assess](#specflowassess) for detailed complexity assessment.

## Overview

### Architecture

The /specflow system orchestrates specialized AI agents through seven distinct phases, managing the transition from high-level requirements to production-ready systems:

```mermaid
graph TB
    A[User Requirements] --> B[/specflow:assess]
    B --> C{Complexity?}
    C -->|Complex| D[/specflow:specify]
    C -->|Medium| D
    C -->|Simple| SKIP[Direct Implementation]

    D --> E[/specflow:research]
    E --> F[/specflow:plan]
    F --> G[/specflow:implement]
    G --> H[/specflow:validate]
    H --> I[/specflow:operate]
...
The /specflow commands are divided into two categories:

**Design Commands** (Create Implementation Tasks):
- `/specflow:specify` - Creates implementation tasks in backlog.md (section 6 of PRD)
- `/specflow:research` - Creates follow-up implementation tasks based on findings
- `/specflow:plan` - Creates architecture and infrastructure tasks

**Implementation Commands** (Execute Existing Tasks):
- `/specflow:implement` - Requires existing tasks with acceptance criteria
- `/specflow:validate` - Validates task completion and marks tasks Done
- `/specflow:operate` - Implements operational infrastructure from backlog tasks

**Critical Rule**: Design commands produce tasks; implementation commands consume them. Never run `/specflow:implement` without first running `/specflow:specify` to create implementation tasks.
...
- `/specflow:research`: Researcher → Business Validator (validator uses researcher output)
- `/specflow:validate`: QA + Security (parallel) → Tech Writer → Release Manager

**Parallel Execution**:
- `/specflow:plan`: Software Architect + Platform Engineer (independent work, then consolidated)
- `/specflow:implement`: Frontend Engineer + Backend Engineer (when both needed)

Sequential patterns ensure data dependencies; parallel patterns maximize throughput.

## The Seven /specflow Commands
...
**Example Usage**:
```bash
/specflow:assess Add user authentication with OAuth2
```
...
**Reference**: See `.claude/commands/specflow/assess.md` for complete assessment framework.
...
**When to Use**:
- Starting a new feature (after `/specflow:assess` recommends Medium or Complex)
...
**Example Usage**:
```bash
/specflow:specify Build a user dashboard with activity timeline and notifications
```
...
**Reference**: See `.claude/commands/specflow/specify.md` for complete agent context.
...
**Inputs**:
- Feature specification from `/specflow:specify`
...
**Example Usage**:
```bash
/specflow:research OAuth2 authentication providers for enterprise SaaS
```
...
**Reference**: See `.claude/commands/specflow/research.md` for complete agent contexts.
...
**Example Usage**:
```bash
/specflow:plan Multi-tenant SaaS application with microservices architecture
```
...
**Reference**: See `.claude/commands/specflow/plan.md` for complete agent contexts.
...
- Never without first running `/specflow:specify` to create tasks

**Execution Pattern**: Parallel engineers (Phase 1) → Sequential code review (Phase 2)

**Critical Prerequisite**: This command **REQUIRES** existing backlog tasks. It will error if no relevant tasks are found. Run `/specflow:specify` first to create implementation tasks.
...
/specflow:implement User authentication and authorization system
```
...
Please run /specflow:specify first to create implementation tasks.
```

**Reference**: See `.claude/commands/specflow/implement.md` for complete agent contexts.
...
# Run validation
/specflow:validate User authentication feature
```
...
**Reference**: See `.claude/commands/specflow/validate.md` for complete agent contexts.
...
/specflow:operate E-commerce platform with microservices
```
...
**Reference**: See `.claude/commands/specflow/operate.md` for complete agent context.

---

## Specialized Agents

The /specflow system coordinates 15 specialized agents across the development lifecycle. Each agent brings domain expertise, proven frameworks, and best practices to their area of responsibility.
...
**Invoked By**: `/specflow:research` (Phase 1)
...
**Invoked By**: `/specflow:research` (Phase 2, after Researcher)
...
**Invoked By**: `/specflow:plan` (parallel with Platform Engineer)
...
**Invoked By**: `/specflow:plan` (parallel with Software Architect)
...
**Invoked By**: `/specflow:implement` (parallel with Backend Engineer)
...
**Invoked By**: `/specflow:implement` (parallel with Frontend Engineer)
...
**Invoked By**: `/specflow:implement` (optional, when ML components needed)
...
**Invoked By**: `/specflow:implement` (after Frontend Engineer)
...
**Invoked By**: `/specflow:implement` (after Backend Engineer)
...
**Invoked By**: `/specflow:validate` (parallel with Secure-by-Design Engineer)
...
**Invoked By**: `/specflow:validate` (parallel with Quality Guardian)
...
**Invoked By**: `/specflow:validate` (after QA + Security)
...
**Invoked By**: `/specflow:validate` (after Tech Writer, final gate)
...
The /specflow system uses two execution patterns based on agent dependencies and optimization goals:
...
The /specflow system enforces a critical separation between design (task creation) and implementation (task execution):

**Design Commands** (Produce Tasks):
- `/specflow:specify` - PM Planner creates implementation tasks in PRD section 6
- `/specflow:research` - Research and Business Validator create follow-up tasks
- `/specflow:plan` - Architect and Platform Engineer create architecture/infrastructure tasks

**Implementation Commands** (Consume Tasks):
- `/specflow:implement` - Engineers pick tasks from backlog and implement
- `/specflow:validate` - Validators verify task completion
- `/specflow:operate` - SRE implements operational tasks

**Critical Rule**: Never run `/specflow:implement` without first running `/specflow:specify` to create implementation tasks.
...
Tasks flow through states as /specflow commands execute:
...
**Valid Transitions**:
- `/specflow:specify` can run on "To Do" tasks → moves to "Specified"
- `/specflow:research` can run on "Specified" tasks → moves to "Researched"
- `/specflow:plan` can run on "Researched" (or "Specified") tasks → moves to "Planned"
- `/specflow:implement` can run on "Planned" tasks → moves to "In Implementation"
- `/specflow:validate` can run on "In Implementation" tasks → moves to "Validated"
- `/specflow:operate` can run on "Validated" tasks → moves to "Deployed"
- Release Manager marks tasks as "Done" after Definition of Done verified

**Invalid Transitions** (will error):
- Running `/specflow:implement` on "To Do" task (must run `/specflow:specify` first)
- Running `/specflow:validate` on "Planned" task (must run `/specflow:implement` first)
...
**Complexity Assessment** (`/specflow:assess`):
...
# Implement directly (no /specflow commands needed)
...
/specflow:specify User preferences API endpoint
...
/specflow:implement User preferences API

# Engineers work from backlog tasks created by /specflow:specify
...
```bash
# 1. Assess complexity
/specflow:assess Stripe payment integration
# Output: Complex (27/32) → Recommend Full SDD

# 2. Create comprehensive specification
/specflow:specify Stripe payment processing integration
...
/specflow:research Payment processing and PCI compliance requirements
...
/specflow:plan Payment processing microservice with Stripe integration
...
/specflow:implement Stripe payment integration
...
/specflow:validate Stripe payment integration
...
/specflow:operate Stripe payment service infrastructure
...
```bash
# 1. Assess complexity
/specflow:assess Refactor monolith to microservices architecture
# Output: Complex (30/32) → Full SDD required

# 2. Specification (focuses on outcomes and constraints)
/specflow:specify Microservices migration for user and order domains
...
/specflow:plan Microservices architecture and migration strategy
...
/specflow:implement User service extraction (Phase 1)
...
/specflow:validate Microservices migration Phase 1
...
/specflow:operate Microservices platform infrastructure
...
/specflow:assess Product recommendation engine
# Output: Complex (24/32) → Full SDD

/specflow:specify ML-powered product recommendations

# 2. Research and validation
/specflow:research Recommendation algorithms and personalization strategies
...
/specflow:plan Recommendation engine architecture
...
/specflow:implement Recommendation engine ML model and API
...
/specflow:validate Recommendation engine
...
/specflow:operate Recommendation engine ML infrastructure
...
The /specflow workflow system is tightly integrated with backlog.md for comprehensive task lifecycle management.

### Task Creation

**Design Commands Create Tasks**:

1. **`/specflow:specify`** - PM Planner creates implementation tasks in PRD section 6:
   ```bash
...
2. **`/specflow:research`** - Researchers create follow-up tasks:
   ```bash
...
3. **`/specflow:plan`** - Architects and Platform Engineers create design/infrastructure tasks:
   ```bash
   # Architecture tasks
...
```bash
# /specflow:implement discovers tasks
backlog search "authentication" --plain
backlog task list -s "To Do" --plain

# /specflow:validate discovers tasks ready for validation
backlog task list -s "In Progress" --plain
backlog task list -s "Done" --plain

# /specflow:operate discovers operational tasks
backlog search "infrastructure" --plain
backlog task list -l infrastructure --plain
```

**Critical Rule**: `/specflow:implement` will ERROR if no tasks are found. Always run `/specflow:specify` first.
...
Tasks transition through states as /specflow commands execute:
...
Complexity Score?
  ...
  ├─ 13-20 (Medium) → Spec-Light: /specflow:specify → /specflow:implement
...
**Parallel Execution**:
- Use `/specflow:plan` parallel mode (Software Architect + Platform Engineer simultaneously)
- Use `/specflow:implement` parallel mode (Frontend + Backend + AI/ML simultaneously)
- Use `/specflow:validate` Phase 1 parallel mode (QA + Security simultaneously)

**Skip Optional Phases**:
- Skip `/specflow:research` if business case is clear
- Skip `/specflow:plan` for simple features using standard patterns
- Skip `/specflow:validate` for low-risk internal tools (not recommended for production)
...
- Always run `/specflow:validate` for production features
- Never skip security review for features handling sensitive data
- Always create runbooks for operational alerts (`/specflow:operate`)
...
- `/specflow:specify` should link implementation tasks to each other
...
**Pitfall 1: Running /specflow:implement without tasks**

❌ **Wrong**:
```bash
/specflow:implement New authentication system
...
```bash
# First create tasks
/specflow:specify New authentication system
...
/specflow:implement → /specflow:research  # Too late for research
```

✅ **Correct**:
```bash
# Follow natural workflow progression
/specflow:specify → /specflow:research → /specflow:plan → /specflow:implement
```
...
**Problem**: `/specflow:implement` errors because no tasks exist.

**Solution**:
```bash
# 1. Verify you ran /specflow:specify first
backlog task list --plain | grep -i "<feature-keyword>"
...
/specflow:implement <feature-description>
```
...
The /specflow workflow can be customized via `specflow_workflow.yml` configuration:
...
The /specflow agents can be accessed via MCP (Model Context Protocol):
...
The /specflow workflow integrates with CI/CD pipelines:
...
## Diagram: /specflow Workflow

The following diagram shows the complete /specflow workflow with agent coordination and backlog integration:

![specflow Workflow Diagram](../diagrams/specflow-workflow.png)
...
### Command References
- **`/specflow:assess`**: `.claude/commands/specflow/assess.md`
