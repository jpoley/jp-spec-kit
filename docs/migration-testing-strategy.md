...
This document outlines the comprehensive testing strategy for the specflow → specflow migration. The migration affects 900+ occurrences across 100+ files and requires careful validation to ensure system integrity.
...
```bash
# Create backup
./scripts/bash/migrate-specflow-to-specflow.sh --dry-run  # Creates backup location
...
**Checks Performed**:
- ✅ No remaining specflow references (except allowed locations)
