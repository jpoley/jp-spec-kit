...
class ToolManager:
    ...
    def _install_binary(self, config: ToolConfig) -> InstallResult:
        ...
        try:
            ...
            # FIX #10: Validate file before chmod
            if not tool_path.is_file() or not tool_path.exists():
                return InstallResult(
                    success=False,
    ...
    def _safe_extract_archive(self, archive_path: Path, dest_dir: Path) -> None:
        ...
        # FIX #7: Use .suffix.lower() for extension checking
        if archive_path.suffix.lower() == ".zip":
            with zipfile.ZipFile(archive_path, "r") as zf:
                # FIX #3: Check file count to prevent zip bomb DoS
                ...
                for member in zf.namelist():
                    ...
                    # FIX #1: Use Path.parts for path traversal check (not substring)
                    if ".." in Path(member).parts:
                        raise RuntimeError(f"Archive contains path traversal: {member}")

                    # FIX #2: Use relative_to() for path validation (not string prefix)
            ...
            # FIX #6: Add file count limit for post-extraction validation
            file_count = 0
            ...
            for item in dest_dir.rglob("*"):
                ...
                # FIX #5: Use relative_to() instead of string prefix matching
            ...
            for item in self.cache_dir.iterdir():
                if item.is_dir():
                    try:
                        ...
                        # FIX #11: Add explanatory comment to except clause
    ...
    def _find_in_cache(self, tool_name: str) -> Path | None:
        ...
        try:
            ...
            # FIX #12: Add explanatory comment to except clause
            ...
            for dirpath, _, filenames in os.walk(path):
                for filename in filenames:
                    ...
                    try:
                        ...
                        # FIX #12: Add explanatory comment
