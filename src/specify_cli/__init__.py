...
def write_repo_facts(project_path: Path, facts: dict) -> None:
    """Write repository facts to memory/repo-facts.md.
    ...
    facts_path = memory_dir / "repo-facts.md"

    # Build YAML frontmatter
    frontmatter = {
        "detected_at": datetime.now().isoformat(),
        "spec_kit_version": __version__,
        "languages": facts.get("languages", []),
        "frameworks": facts.get("frameworks", []),
        "ci_cd": facts.get("ci_cd"),
        "test_framework": facts.get("test_framework"),
        "linter": facts.get("linter"),
        "formatter": facts.get("formatter"),
        "security_tools": facts.get("security_tools", []),
    }
    ...
    content += "This file is auto-generated by `/speckit:constitution` and updated by `specify init` / `specify upgrade`.\n"
    content += "Edit only if LLM detection was incorrect.\n\n"
    ...
    if facts.get("ci_cd"):
        content += "## CI/CD\n"
        content += f"- {facts['ci_cd']}\n\n"

    if facts.get("test_framework"):
        content += "## Testing\n"
        content += f"- {facts['test_framework']}\n\n"

    if facts.get("linter") or facts.get("formatter"):
        content += "## Code Quality\n"
        if facts.get("linter"):
            content += f"- Linter: {facts['linter']}\n"
        if facts.get("formatter"):
            content += f"- Formatter: {facts['formatter']}\n"
        content += "\n"

    if facts.get("security_tools"):
        content += "## Security\n"
        for tool in facts["security_tools"]:
            content += f"- {tool}\n"
        content += "\n"
...
def init(
    ...
    with Live(
        ...
        try:
            ...
            # Detect and write repository facts
            ...
            try:
                ...
                if (project_path / ".github" / "workflows").exists():
                    ...
                    facts["ci_cd"] = "GitLab CI"

                if facts:
                    write_repo_facts(project_path, facts)
                    tracker.complete("repo-facts", "detected and saved")
                else:
                    tracker.skip("repo-facts", "no facts detected")
            except Exception as facts_error:
                tracker.error("repo-facts", f"detection failed: {facts_error}")
