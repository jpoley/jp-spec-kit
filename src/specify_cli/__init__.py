...
def _get_repo_facts_template() -> str:
    """Return the repo-facts.md template with current date."""
        ...
        try:
            ...
            try:
                ...
                if selected_constitution in CONSTITUTION_TEMPLATES:
                    ...
                    repo_facts_dest = memory_dir / "repo-facts.md"
                    memory_dir.mkdir(parents=True, exist_ok=True)
                    constitution_dest.write_text(
                        CONSTITUTION_TEMPLATES[selected_constitution]
                    )
                    # Create repo-facts.md placeholder if it doesn't exist
                    if not repo_facts_dest.exists():
                        repo_facts_template = _get_repo_facts_template()
                        repo_facts_dest.write_text(repo_facts_template)
                    tracker.complete(
                        "constitution",
        ...
        if sys.stdin.isatty():
            ...
            if add_constitution:
                ...
                if not repo_facts_dest.exists():
                    ...
                    repo_facts_dest.write_text(repo_facts_template)
                    console.print(
                        "[green]✓[/green] Repo facts placeholder: [cyan]memory/repo-facts.md[/cyan]"
                    )
                console.print(
...
constitution_app = typer.Typer(
    name="constitution",
    help="Validate and manage constitution files",
    add_completion=False,
)
...
def constitution_validate(
    ...
    Scans the constitution file for NEEDS_VALIDATION markers and reports
    which sections still require manual review and validation.

    Exit codes:
        0: All sections validated (no NEEDS_VALIDATION markers found)
        1: Unvalidated sections found
        2: File not found or other error

    Example:
        specify constitution validate
        specify constitution validate --path custom/constitution.md
        specify constitution validate --json  # Machine-readable output
    """
    import re
    ...
    if path is None:
        path = Path.cwd() / "memory" / "constitution.md"
    ...
    if not path.exists():
        if json_output:
            ...
            print(
                json.dumps(
                    {"error": "Constitution file not found", "path": str(path)},
                    indent=2,
                )
            )
        else:
            console.print(
                f"[red]Error:[/red] Constitution file not found at [cyan]{path}[/cyan]"
            )
            console.print(
                "\n[dim]Run 'specify init' to create a constitution file.[/dim]"
            )
        raise typer.Exit(2)
    ...
    try:
        ...
        if json_output:
            ...
            print(
                json.dumps(
                    {"error": f"Failed to read constitution: {e}", "path": str(path)},
                    indent=2,
                )
            )
        else:
    ...
    # Pattern to match NEEDS_VALIDATION markers
    # Matches: <!-- NEEDS_VALIDATION: description -->
    validation_pattern = re.compile(r"<!--\s*NEEDS_VALIDATION:\s*(.+?)\s*-->")

    # Find all NEEDS_VALIDATION markers with their context
    ...
    for line_num, line in enumerate(lines, start=1):
        ...
        if match:
            ...
            # Try to find the section heading (look backwards for ## or ###)
    ...
    if json_output:
        ...
        result = {
            "path": str(path),
            "validated": len(markers) == 0,
            "unvalidated_count": len(markers),
            "markers": markers,
        }
        print(json.dumps(result, indent=2))
    else:
        # Human-readable output
        console.print("\n[bold]Constitution Validation Status[/bold]")
        console.print("=" * 40)

        if len(markers) == 0:
            console.print("\n[green]✓ Constitution fully validated![/green]")
            console.print(
                "\nAll sections have been reviewed. No NEEDS_VALIDATION markers found."
            )
            raise typer.Exit(0)

        # Show unvalidated sections
        console.print(
            f"\n[yellow]⚠ {len(markers)} section(s) need validation:[/yellow]\n"
        )

        for marker in markers:
            console.print(f"[yellow]✗[/yellow] [bold]{marker['section']}[/bold]")
            console.print(
                f"  [dim]Line {marker['line']}:[/dim] {marker['description']}"
            )
            console.print()

        # Show instructions
        console.print("[cyan]Next steps:[/cyan]")
        console.print("1. Review each section marked with NEEDS_VALIDATION")
        console.print("2. Update the content to match your project needs")
        console.print("3. Remove the <!-- NEEDS_VALIDATION: ... --> comment")
        console.print("4. Run this command again to verify\n")

        console.print(f"[dim]Edit file: {path}[/dim]\n")

    # Exit with appropriate code
    raise typer.Exit(1 if len(markers) > 0 else 0)
