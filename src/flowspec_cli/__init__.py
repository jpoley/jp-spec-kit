        ...
        try:
            ...
            try:
                ...
                if has_github_pr_safety_config(project_path):
                    ...
                    # Use CLI flags if provided, otherwise prompt or use defaults
                    pr_merge_setting = block_pr_merge
                    pr_updates_setting = block_pr_updates

                    if pr_merge_setting is None or pr_updates_setting is None:
                        if no_pr_safety_prompts:
                            # Use secure defaults
                            pr_merge_setting = (
                                True if pr_merge_setting is None else pr_merge_setting
                            )
                            pr_updates_setting = (
                                True
                                if pr_updates_setting is None
                                else pr_updates_setting
                            )
                        elif sys.stdin.isatty():
                            # Temporarily exit Live context for prompts
                            live.stop()
                            pr_merge_setting_prompt, pr_updates_setting_prompt = (
                                prompt_github_pr_safety(
                                    console,
                            ...
                            pr_merge_setting = (
                                pr_merge_setting
                                if pr_merge_setting is not None
                                else pr_merge_setting_prompt
                            )
                            pr_updates_setting = (
                                pr_updates_setting
                                if pr_updates_setting is not None
                                else pr_updates_setting_prompt
                            )
                            console.print()
                            live.start()
                        else:
...
def upgrade_repo(
    ...
    if not has_github_pr_safety_config(project_path):
        ...
        # Determine settings from CLI flags or prompts
        pr_merge_setting = block_pr_merge
        pr_updates_setting = block_pr_updates
        ...
        if pr_merge_setting is None or pr_updates_setting is None:
            if no_pr_safety_prompts:
                ...
                pr_updates_setting = (
                    True if pr_updates_setting is None else pr_updates_setting
                )
            elif sys.stdin.isatty():
                # Interactive prompts
                pr_merge_setting_prompt, pr_updates_setting_prompt = (
