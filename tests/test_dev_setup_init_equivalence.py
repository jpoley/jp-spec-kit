...
class TestDevSetupInitEquivalence:
    ...
    def test_init_would_copy_same_files(
        ...
        if specflow_commands.exists():
            for symlink in specflow_commands.glob("*.md"):
                if symlink.is_symlink() and _is_active_template(symlink.name):
                    dev_setup_files.add(f"specflow/{symlink.name}")
...
class TestDevSetupIdempotency:
    ...
    def test_symlink_count_stable(self, claude_commands_dir: Path) -> None:
        ...
        specflow_symlinks = [f for f in specflow_dir.glob("*.md") if f.is_symlink()]
        speckit_symlinks = [f for f in speckit_dir.glob("*.md") if f.is_symlink()]

        assert len(specflow_symlinks) > 0, "No specflow symlinks found"
