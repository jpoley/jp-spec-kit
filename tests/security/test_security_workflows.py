...
class TestSecurityScanReusableWorkflow:
    ...
    def test_workflow_inputs(self, workflow_data):
        ...
        # scan-type defaults to 'full' (CLI currently runs all scans as full)
...
class TestSecurityConfiguration:
    ...
    def test_fail_on_supports_config_fallback(self):
        """Test that fail-on can use config file when input is empty."""
        ...
        # fail-on should reference inputs.fail-on, which can be empty (use config)
        fail_on = inputs.get("fail-on", "")
        # Should be a GitHub Actions expression referencing inputs.fail-on
        assert "${{ inputs.fail-on }}" in str(fail_on) or fail_on == ""
