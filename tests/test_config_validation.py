"""Tests for specify config validation command."""
...
class TestConfigValidationColorOutput:
    ...
    def test_none_mode_shows_green(self, initialized_project: Path) -> None:
        """Test NONE mode displayed in green."""
        result = runner.invoke(app, ["config", "validation", "--show"])
        assert result.exit_code == 0
        # Rich output will include color codes
        assert "NONE" in result.output

    def test_keyword_mode_shows_yellow(self, initialized_project: Path) -> None:
        """Test KEYWORD mode displayed in yellow after update."""
    ...
    def test_pull_request_mode_shows_blue(self, initialized_project: Path) -> None:
        """Test PULL_REQUEST mode displayed in blue after update."""
