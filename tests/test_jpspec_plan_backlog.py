...
def plan_command_path():
    ...
    return (
        Path(__file__).parent.parent / ".claude" / "commands" / "jpspec" / "plan.md"
    )
...
class TestPlanCommandStructure:
    ...
    def test_plan_includes_backlog_instructions(self, plan_command_path):
        ...
        assert includes == 2, f"Expected 2 backlog instruction includes, found {includes}"
