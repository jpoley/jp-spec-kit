"""Tests for /specflow:plan command backlog.md integration.

This test module verifies that the /specflow:plan command correctly integrates
...
def plan_command_path():
    ...
    return Path(__file__).parent.parent / ".claude" / "commands" / "specflow" / "plan.md"
...
class TestPlanCommandStructure:
    ...
    def test_plan_command_has_task_discovery_section(self, plan_command_path):
        ...
        has_include_pattern = (
            "{{INCLUDE:.claude/commands/specflow/_workflow-state.md}}" in content
        )
...
class TestIntegrationScenarios:
    ...
    def test_plan_command_supports_architecture_workflow(self, plan_command_path):
        ...
        has_task_section = (
            ...
            or "{{INCLUDE:.claude/commands/specflow/_workflow-state.md}}" in content
        )
    ...
    def test_both_agents_receive_shared_instructions(self, plan_command_path):
        ...
        for line in content.split("\n"):
            if "{{INCLUDE:.claude/commands/specflow/_backlog-instructions.md}}" in line:
