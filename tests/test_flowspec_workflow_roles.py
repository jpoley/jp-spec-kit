"""Tests for Flowspec Workflow v2.0 with roles configuration.

Tests cover:
- Loading flowspec_workflow.yml with roles section
...
- Backwards compatibility with flowspec_workflow.yml
"""
...
@pytest.fixture
def flowspec_config_path(tmp_path: Path) -> Path:
    """Create a test flowspec_workflow.yml v2.0 with roles."""
    config_data = {
        "version": "2.0",
        "roles": {
            "primary": "dev",
...
class TestFlowspecWorkflowRoles:
    """Tests for v2.0 roles configuration."""

    def test_loads_flowspec_config_with_roles(
        self, flowspec_config_path: Path, roles_schema_path: Path
    ):
        """Test loading flowspec_workflow.yml v2.0 with roles section."""
        config = WorkflowConfig.load(
            path=flowspec_config_path,
...
class TestBackwardsCompatibility:
    """Tests for backwards compatibility with flowspec_workflow.yml v1.x."""

    def test_loads_legacy_flowspec_workflow_without_roles(
        self, tmp_path: Path, roles_schema_path: Path
    ):
        """Test that v1.x flowspec_workflow.yml without roles still loads."""
        config_data = {
            "version": "1.1",
            # No roles section
            "states": ["To Do", "Done"],
    ...
    def test_loads_flowspec_workflow_config(self, tmp_path: Path):
        """Test that flowspec_workflow.yml is loaded correctly."""
        flowspec_config = {
            "version": "2.0",
        ...
        flowspec_path = tmp_path / "flowspec_workflow.yml"

        with open(flowspec_path, "w") as f:
            yaml.dump(flowspec_config, f)
        ...
        try:
            ...
            # Should load flowspec_workflow.yml (v2.0)
            assert config.version == "2.0"
            assert "roles" in config._data
        finally:
