"""Integration tests for specflow command suite with backlog.md CLI.

This test module verifies that all specflow commands (research, validate, specify, plan, implement)
...
def research_command_file():
    ...
    command_path = (
        Path(__file__).parent.parent / ".claude" / "commands" / "specflow" / "research.md"
    )
...
class TestSharedBacklogInstructions:
    ...
    def test_both_agents_have_include_before_context(self, research_command_file):
        ...
        phase2_include = content.find(
            "{{INCLUDE:.claude/commands/specflow/_backlog-instructions.md}}", phase2_start
        )
