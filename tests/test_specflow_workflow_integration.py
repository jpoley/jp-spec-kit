"""Integration tests for /specflow workflow state constraints.

This test module verifies that /specflow commands correctly enforce workflow
state constraints through the WorkflowStateGuard module. It covers:

- AC #1: Test file exists
- AC #2-7: State transitions for all /specflow commands
...
def default_workflow_config():
    """Standard workflow configuration matching specflow_workflow.yml."""
    return {
        "version": "1.1",
        "states": [
            "To Do",
...
def test_integration_test_file_exists():
    ...
    assert test_file.name == "test_specflow_workflow_integration.py"


# =============================================================================
# AC #2: /specflow:specify State Transition (Assessed → Specified)
# =============================================================================


class TestSpecifyTransition:
    """AC #2: Tests for /specflow:specify state transition (Assessed → Specified)."""
    ...
    def test_specify_blocked_from_todo(self, workflow_config_file):
        ...
        assert "Assessed" in response.required_states
        assert "/specflow:specify" in response.message
...
# =============================================================================
# AC #3: /specflow:research State Transition (Specified → Researched)
# =============================================================================


class TestResearchTransition:
    """AC #3: Tests for /specflow:research state transition (Specified → Researched)."""
...
# =============================================================================
# AC #4: /specflow:plan State Transition (Specified/Researched → Planned)
# =============================================================================


class TestPlanTransition:
    """AC #4: Tests for /specflow:plan state transition (Specified/Researched → Planned)."""
...
# =============================================================================
# AC #5: /specflow:implement State Transition (Planned → In Implementation)
# =============================================================================


class TestImplementTransition:
    """AC #5: Tests for /specflow:implement state transition (Planned → In Implementation)."""
    ...
    def test_implement_blocked_from_specified(self, workflow_config_file):
        ...
        # Should suggest /specflow:plan
        assert response.suggested_workflows is not None
        assert "/specflow:plan" in response.suggested_workflows
    ...
    def test_implement_error_message_quality(self, workflow_config_file):
        ...
        assert "Cannot run /specflow:implement" in response.message
...
# =============================================================================
# AC #6: /specflow:validate State Transition (In Implementation → Validated)
# =============================================================================


class TestValidateTransition:
    """AC #6: Tests for /specflow:validate state transition (In Implementation → Validated)."""
    ...
    def test_validate_suggests_implement_workflow(self, workflow_config_file):
        """Validate command suggests /specflow:implement when task is Planned."""
        guard = WorkflowStateGuard(workflow_config_file)
        response = guard.check_state("validate", "Planned")

        assert response.suggested_workflows is not None
        assert "/specflow:implement" in response.suggested_workflows


# =============================================================================
# AC #7: /specflow:operate State Transition (Validated → Deployed)
# =============================================================================


class TestOperateTransition:
    """AC #7: Tests for /specflow:operate state transition (Validated → Deployed)."""
    ...
    def test_operate_suggests_validate_workflow(self, workflow_config_file):
        """Operate command suggests /specflow:validate when task is In Implementation."""
        guard = WorkflowStateGuard(workflow_config_file)
        response = guard.check_state("operate", "In Implementation")

        assert response.suggested_workflows is not None
        assert "/specflow:validate" in response.suggested_workflows
...
class TestInvalidTransitions:
    ...
    def test_blocked_transition_suggests_valid_workflows(self, workflow_config_file):
        ...
        # For "To Do" state, only /specflow:assess is valid
        assert response.suggested_workflows is not None
        assert "/specflow:assess" in response.suggested_workflows
...
class TestCustomWorkflowConfig:
    ...
    def test_custom_config_suggestions(self, custom_config_file):
        ...
        assert "/specflow:start" in workflows

        workflows = guard.get_valid_workflows_for_state("Active")
        assert "/specflow:finish" in workflows
...
class TestEdgeCases:
    ...
    def test_assess_workflow_at_start(self, workflow_config_file):
        ...
        assert valid_workflows == ["/specflow:assess"]
