#!/usr/bin/env python3
"""Validate jpspec_workflow.yml against the JSON Schema.
...
Usage:
    ...
    If not specified:
    - workflow_file defaults to: jpspec_workflow.yml
    - schema_file defaults to: memory/jpspec_workflow.schema.json
...
def main() -> int:
    ...
    workflow_file = args[0] if len(args) > 0 else "jpspec_workflow.yml"
    schema_file = args[1] if len(args) > 1 else "memory/jpspec_workflow.schema.json"
    ...
    print("=" * 70)
    print("jpspec_workflow.yml Validation")
    print("=" * 70)
