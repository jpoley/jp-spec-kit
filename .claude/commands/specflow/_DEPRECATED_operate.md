---
description: "[DEPRECATED] Use /ops:deploy instead - Execute operations workflow using SRE agent for CI/CD, Kubernetes, DevSecOps, observability, and operational excellence."
deprecated: true
replacement: "/ops:deploy"
...
Simply replace `/specflow:operate` with `/ops:deploy` in your workflows. The functionality is identical.

**Before**:
```bash
/specflow:operate my-feature
```

**After**:
```bash
/ops:deploy my-feature
```
...
**Redirecting to /ops:deploy...**

{{INCLUDE:.claude/commands/ops/deploy.md}}
