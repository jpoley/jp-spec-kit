...
def run_hook(tool_name: str, tool_input: dict = None, env_overrides: dict = None) -> dict:
...
def test_merge_asks_for_main_protection():
    """When merge is allowed but main protection is on, should ask."""
    print("Test: Merge asks for main protection...", end=" ")
...
def test_create_review_blocked_by_default():
    ...
    response = run_hook(
        ...
        {"owner": "test", "repo": "test", "pull_number": 1, "body": "LGTM", "event": "APPROVE"},
    )
...
def test_config_file_respected():
    ...
    try:
        ...
        assert response.get("decision") == "allow", f"Expected allow with permissive config, got {response}"
...
def test_env_overrides_config():
    ...
    assert response.get("decision") == "deny", f"Expected deny with env override, got {response}"
