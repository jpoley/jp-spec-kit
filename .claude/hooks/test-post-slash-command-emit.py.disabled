...
Tests the auto-emit functionality for /flowspec slash commands.
"""
...
class TestPostSlashCommandEmit(unittest.TestCase):
    ...
    def test_non_flowspec_command_allows(self):
        """Non-flowspec slash commands should be allowed without emission."""
        exit_code, output = run_hook(
            {
        ...
        self.assertEqual(exit_code, 0)
        self.assertEqual(output["decision"], "allow")
        self.assertEqual(output["reason"], "Not a /flowspec command")

    def test_flowspec_assess_detected(self):
        """Test /flow:assess is detected and maps to workflow.assessed."""
        exit_code, output = run_hook(
            {
                "tool_name": "SlashCommand",
                "tool_input": {"command": "/flow:assess my-feature"},
            }
        )
    ...
    def test_flowspec_specify_detected(self):
        """Test /flow:specify is detected and maps to spec.created."""
        exit_code, output = run_hook(
            {
                "tool_name": "SlashCommand",
                "tool_input": {"command": "/flow:specify auth-feature --spec-id auth"},
            }
        )
    ...
    def test_flowspec_implement_detected(self):
        """Test /flow:implement is detected and maps to implement.completed."""
        exit_code, output = run_hook(
            {
                "tool_name": "SlashCommand",
                "tool_input": {"command": "/flow:implement task-229"},
            }
        )
    ...
    def test_flowspec_validate_detected(self):
        """Test /flow:validate is detected and maps to validate.completed."""
        exit_code, output = run_hook(
            {
                "tool_name": "SlashCommand",
                "tool_input": {"command": "/flow:validate"},
            }
        )
    ...
    def test_flowspec_operate_detected(self):
        """Test /flow:operate is detected and maps to deploy.completed."""
        exit_code, output = run_hook(
            {
                "tool_name": "SlashCommand",
                "tool_input": {"command": "/flow:operate production"},
            }
        )
    ...
    def test_extract_task_id_from_command(self):
        ...
        exit_code, output = run_hook(
            {
                "tool_name": "SlashCommand",
                "tool_input": {"command": "/flow:implement --task-id task-123"},
            }
        )
    ...
    def test_extract_feature_from_output(self):
        ...
        exit_code, output = run_hook(
            {
                "tool_name": "SlashCommand",
                "tool_input": {"command": "/flow:specify"},
    ...
    def test_all_flowspec_commands_mapped(self):
        """Verify all /flowspec commands have event mappings."""
        commands = [
